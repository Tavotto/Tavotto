...
**回弹只有一条曲线**（2026-09-15，改图助手质感）：`--ease-spring` = `cubic-bezier(0.34, 1.4, 0.64, 1)`，
峰值约 5%，只给「新内容落位」的收尾——对话块 / diff 卡的 `settle-in`、发送 ↔ 中止的图标互换。
幅度跟着位移走（4px 位移上的回弹是 0.2px），它改的是收尾的手感，不是一个看得见的弹跳；退场、hover、
跟随输入的东西一律不用它。JS 侧同一份在 `lib/motion.EASE_SPRING`（motion.test 对拍）。
