...
- **独立 mappable 的色条认得出它描述的图（2026-09-24，用户的 PRB 三联图 (a)：离线渲染好的
  ...
  色图上的不透明像素 ≥ 60%、铺开色图全长 ≥ 10%、norm 可逆），绑成 `RasterField`。
  **原样是模式**：色图 / norm 签名没变时画脚本原件（同一个数组对象，不拷贝）；变了才重着色。
  **反解是按颜色查表，不是按像素求解**：`_ColourTube` 把色图每一格周围 `_FIELD_TOL` 以内的
  全部 8 位颜色列成「颜色 → 最近格」表（大小只跟色图有关），重着色 = N 项的「格 → 新颜色」
  + 按**二维**小块（`_FIELD_TILE`，块像素数与图的宽高比无关）逐块查表写进一份 uint8 缓冲；
  「在色图上」还要连成片（`_field_mask`，八邻域 ≥ 3、封顶到实际邻居数——单行色带只有两个），
  照片里偶然落进容差带的孤点不算；**配对的吻合度问同一个函数**（`_field_fit` 在行列各自
  均匀的连续窗口里抽样，`_sample_windows`），两侧判据不同源就会报已绑定却换不了色。
  查色表只在作用域里有候选位图时才建；按色图的**真实颜色**建（`_cmap_entries`：按色图自己的
  N 格取样、8 位去重、同色取平台中点——离散调色板的每一种颜色都在，连续色图再细也只剩几百
  上千种；**分段取样、边取边去重、超上限即停**，峰值与 N 无关，N 超 2²⁴ 只看格数就不配对），
  多段建表逐段取最近格、总耗时线性；去重后超过 `_FIELD_MAX_COLOURS`（16384）种的调色板不配对。
  叠加物原样，它们的抗锯齿边缘在同一轮分块里逐块现算、逐块应用，按「离底色距离 / 邻域线芯
  距离（下限 0.6）」带过底色变化；逐块结果只在总量不超 `_FIELD_EDGE_CACHE`（16 MiB）时留作
  下次换色的缓存，超了整份放弃、每次逐块重算（#538 评审第六轮：密集阴影线的图边缘占三分之一，
  按与图同长的表常驻峰值斜率 21.9 B/像素）。常驻内存 = 输出本身（3–4 B/像素）+ 有上限的
  边缘缓存；32 MiB 的直查表每次现建、不常驻。**没有像素数上限、也不先数唯一
  ...
  **认领范围（#527 评审 P1）**：①② 都只在色条声明的宿主（`_colorbar_info["parents"]`）
  ...
  一个图元只归一条色条；「作用域里已有图元共用这个 norm 就不再认领 / 配对」也只看本作用域
  （#538 评审第七轮：同一个 ScalarMappable 交给左右两个子图各建一条色条，全局判时左边认领
  过的图挡住了右边）。（#527 评审 P2 与 #538 评审两轮 P2——嵌照片的图报已绑定却反解失败、
  全图数颜色的内存、按行分块的极宽图——根子都在旧的「按唯一颜色暴力求最近格」，改成查表后
  这三件事一起消失。）manifest
