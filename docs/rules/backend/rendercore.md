...
- **冷启动预热**（ADR 0077）：`pdfbackend.warm()` 装载实现之后，实现若有 `prewarm()` 就交给它；候选在后台线程里起 child、建字体注册表、
  ...
  `_stop_prewarm`（#641）：原生那一段（字体注册表 + 第一次排版：import pikepdf / uharfbuzz，pikepdf 是 nanobind）只在锁里、先查取消标志再进；
  回调立取消标志再拿一次那把锁——返回之后线程要么做完了原生那段、要么永远不进去。**不是有界 join**：ping 能等到 child 超时（60 s），
  join 超时后线程照样会走出 ping 进原生段（Codex #644）。解释器收尾时 daemon 线程被强制退出、穿过 C++ 栈帧，py3.10 Linux 上 abort / SIGSEGV；
  atexit 跑在收尾之前，还停在 ping 里的线程只在标准库的锁 / 管道上等。启动时起的其它 daemon 线程
