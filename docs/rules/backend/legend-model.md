...
- **自定义 handler 画的整格定格复刻（2026-09-24，用户 Figure2）**：matplotlib 不保存
  ...
  整格复刻（#544 评审）；跟随中的照旧从源派生（有源的误差棒），断开跟随时换回的是整格。定格项
  脚本原样整格同一种颜色的（误差棒，`frozen_color_uniform`）仍给 `handle_color`，写到这一格**每个**
  artist；颜色不统一的（色带）不给 handle_*。示意线指纹带**未缩放的虚线节奏**（`_dash_key`）：
