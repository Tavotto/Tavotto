...
- **自报只收这一条会话的（U09，ADR 0070）**：worker 的 `runtime_report()` 带 `report_origin=build` 与 `pid`；
  ...
  `runtime_rejected ∈ {not_a_build_report, pid_mismatch}`、`completeness=partial`；不知道 pid 就 `pid_check=unavailable`，
...
- **阶段轨迹（U09，ADR 0071）**：`engine/trace.py` 的 `Trace` 有界（64 条，丢中间留头尾）、阶段名闭集、`facts` 只收标量、
  ...
  source → compile → compose → raster → inspect → publish → report）同一个形状，随 `to_payload()` 走。不是全系统追踪平台。
