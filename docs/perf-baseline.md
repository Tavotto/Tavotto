...
A = 上限形同不存在 / B = 24 MB，逐次轮流跑，取中位），机器 **load average ≈ 10**
（十几个 agent 并行），所以绝对值只在同一次跑内部可比：
...
复核重跑（load average ≈ 3，独立脚本名）：5 000 对象 614.8 → 218.4 ms（2.81×），
文件 70.9 → 24.4 MB、61 → 21 条，与上表一致。
