...
lines = [l for l in (p.stdout+p.stderr).splitlines() if "INFO werkzeug:" not in l]
with open(os.path.join(LOGS,name+".log"),"a" if append else "w",encoding="utf-8") as f:
    f.write(f"# verify: {name}\n# time: {t0.isoformat(timespec='seconds')} (+{(datetime.datetime.now()-t0).total_seconds():.0f}s)\n# HEAD: {sha}\n# cwd: <verify worktree root>\n$ {cmd}\n")
    f.write("\n".join(lines)+"\n")
    f.write(f"[exit={p.returncode}]\n\n")
tail=[l for l in lines if any(k in l for k in ("passed","failed","Tests ","Test Files","skipped","BAD","error","Error"))]
print(f"{name}: exit={p.returncode}"); print("\n".join(tail[-12:]))
