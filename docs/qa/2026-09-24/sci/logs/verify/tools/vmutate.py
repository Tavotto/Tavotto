...
W=Path("/private/tmp/claude-501/-Volumes-Projects-Tavotto/47aff04a-e334-4d42-be51-2f032b92dc64/scratchpad/qa/verify-sci")
...
assert sep=="--"
p=W/target; src=p.read_text(encoding="utf-8")
assert src.count(old)==int(count),(src.count(old))
assert not subprocess.run(["git","status","--porcelain","--",target],cwd=W,capture_output=True,text=True).stdout.strip()
cmd="bash docs/qa/2026-09-24/sci/repro/pytest.sh "+" ".join(args)
p.write_text(src.replace(old,new),encoding="utf-8")
try:
    subprocess.run(["bash",V,case,f"mutation-{label}","--","git --no-pager diff; "+cmd],cwd=W)
finally:
...
print("status:",subprocess.run(["git","status","--porcelain"],cwd=W,capture_output=True,text=True).stdout.strip() or "clean")
