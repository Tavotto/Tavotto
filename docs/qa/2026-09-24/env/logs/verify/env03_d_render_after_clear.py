...
rec = p.load_e1(E1); A = rec["envs"]["A"]; proj = E1 / "proj"
p.native_reference(rec["pythons"]["A"], proj, OUT)
site_a = Path(A["pkg_file"]).parent.parent
p.reset_project_state(proj)
work = OUT / "d_extra"; shutil.rmtree(work, ignore_errors=True)
ddir = OUT / "d_extra_env"; shutil.rmtree(ddir, ignore_errors=True)
...
with p.app_for(proj, work, {}) as app:
    ...
    st, body = app.call("/api/engine/environment", {"python": p.py_of(ddir), "scope": "project"}, method="PATCH", timeout=180)
    s = p.prepare(app, panel["id"]); print("D chosen:", s["result"]["status"], s["plan"]["environment"].get("source"))
    (site_d / "_qa_link_a.pth").unlink()
with p.app_for(proj, work, {}) as app:
    panel = p.panel_of(app, "figure.pdf")
    s = p.prepare(app, panel["id"]); print("D broken:", s["result"]["status"], (s["result"].get("error") or {}).get("code"))
    app.call("/api/engine/environment", {"python": "", "scope": "project"}, method="PATCH", timeout=120)
    _, envst = app.call("/api/engine/environment", timeout=60)
    print("environment.project after clear:", json.dumps(envst.get("project"), ensure_ascii=False, default=str)[:1500])
    s = p.prepare(app, panel["id"])
    print("prepare after clear:", s["result"]["status"], json.dumps(s["plan"].get("environment"), ensure_ascii=False, default=str)[:1500])
    try:
        r = p.render(app, panel["id"])
        print("render after clear OK; values:", p.values_from(r), "ident:", p.ident_from(r))
        ok = p.values_from(r) == A["value"]
        rc = r.get("receipt") or {}
        print("render receipt:", {k: rc.get(k) for k in ("python_source", "generation", "pid_check")})
        _, envst2 = app.call("/api/engine/environment", timeout=60)
        pj = envst2.get("project") or {}
        print("environment.project after render:", {k: pj.get(k) for k in ("source", "trigger", "automatic", "python")})
        lg = app.server_log()
        ...
        print("render after clear FAILED:", json.dumps(exc.body, ensure_ascii=False, default=str)[:1500]); ok = False
