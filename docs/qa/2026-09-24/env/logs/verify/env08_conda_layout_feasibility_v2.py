"""v2：脚本另 import openpyxl（能映射到 distribution → 联合计划 ready，ADR 0079 的门才会看用户环境）。
...
REPRO = Path(sys.argv[1]); E1 = Path(sys.argv[2]); OUT = Path(sys.argv[3])
sys.path.insert(0, str(REPRO))
import e1_http_probe as p  # noqa
import make_e1 as m  # noqa
p.fa.free_port = lambda: 5403
rec = p.load_e1(E1); base = rec["base"]
d = OUT / "ENV-08b"; shutil.rmtree(d, ignore_errors=True)
proj = d / "proj"; proj.mkdir(parents=True)
(proj / "figure.py").write_text("import openpyxl  # noqa: F401  映射得到的 PyPI 包（计划 ready）\n" + m.FIGURE_PY, encoding="utf-8")
work = d / "work"; home = work / "home"
conda_base = home / "miniforge3"; lab = conda_base / "envs" / "lab"
whl = E1 / "wheels" / "qa_probe_pkg-1.0.0-py3-none-any.whl"
m.make_venv(conda_base, base, m.MPL_SPEC)
m.make_venv(lab, base, m.MPL_SPEC, str(whl), "openpyxl")
...
with p.app_for(proj, work, {}) as app:
    ...
    print("prepare:", st["result"]["status"], json.dumps(st["plan"].get("environment"), ensure_ascii=False, default=str)[:1500])
    (OUT / "ENV-08b" / "prepare_state.json").write_text(json.dumps(st, ensure_ascii=False, indent=1, default=str))
    print("prepare result error:", json.dumps(st["result"].get("error"), ensure_ascii=False, default=str)[:800])
    _, dep = app.call("/api/engine/dependencies?script=figure.py", timeout=120)
    (OUT / "ENV-08b" / "dependencies.json").write_text(json.dumps(dep, ensure_ascii=False, indent=1, default=str))
    ofr = (dep or {}).get("offer") or {}
    print("offer.user_environments:", json.dumps(ofr.get("user_environments"), ensure_ascii=False, default=str)[:1200])
    print("offer.plan.missing/unknown:", json.dumps({k: (ofr.get("plan") or {}).get(k) for k in ("missing", "unknown")}, ensure_ascii=False, default=str)[:800])
    ...
    try:
        ...
        ident = p.ident_from(r); vals = p.values_from(r)
        ...
        print("render FAILED:", json.dumps(exc.body, ensure_ascii=False, default=str)[:1500]); ok = False
    _, envst = app.call("/api/engine/environment", timeout=60)
    pj = envst.get("project") or {}
    print("environment.project:", {k: pj.get(k) for k in ("source", "source_label", "trigger", "automatic", "python")})
