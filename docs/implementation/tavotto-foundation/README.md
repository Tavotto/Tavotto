...
全路径全过；`u08_parity` 654 / 0 / 13 skip；闭包 wheel 表（`scripts/dev/u10_wheel_matrix.py`）正式三格 75/75（合入 main 时复核：pikepdf 10.x 的 wheel 要求 macOS 14（arm64）/ 15（x86_64），
桌面版最低系统随之抬到 14 / 15，Intel 桌面版保留，见 ADR 0072 §2）；本机冻结 child 7/7。然后一个可审查变更：`BACKEND_DEFAULT = rendercore`、`pymupdf_backend.py` 删除、`pymupdf` 只剩
