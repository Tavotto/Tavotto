...
| `CANVAS_TEXT_FAMILIES` | `identity` | public_contract | `scripts/gen_glyph_plan_vectors.py:59` · `<module>` | `tests/test_typography_families.py:28` (user_contract)<br>`tests/test_glyph_plan.py:117` (user_contract)<br>`tests/test_font_provenance.py:130` (user_contract)<br>`tests/test_scientific_text_matrix.py:154` (user_contract) | — | U06/U08：闭集**是一句能力承诺**——新后端画得出什么，闭集就是什么，不能照抄（facade 文档原话）。三个通用族 + 粗 / 斜 / 粗斜必须由批准字体集合兑现；前端摆得出的后端必须画得出（test_typography_families 保留）。 |
...
| `probe_asset` | `probe` | public_contract | `src/tavotto/app.py:552` · `scan_panels`<br>`src/tavotto/app.py:1206` · `_original_page_pt`<br>`src/tavotto/app.py:1222` · `_declared_density`<br>`src/tavotto/app.py:4172` · `_post_check_size`<br>`src/tavotto/engine/artifactcheck.py:80` · `check_pdf`<br>`src/tavotto/engine/artifactcheck.py:149` · `check_raster`<br>`src/tavotto/engine/tutorial.py:573` · `validate_tutorial_resources`<br>`codex-plugin/mcp/tavotto_mcp/bridge.py:1902` · `_original_artifact_facts` | `tests/test_original_spec.py:122` (user_contract)<br>`tests/test_export_pipeline.py:786` (user_contract)<br>`tests/test_mcp_normalize.py:282` (user_contract) | — | U08：八个调用点一个不漏（RC-001）；返回结构逐键相同；`alpha` 真值来自新读取器；密度仍由 originalspec 解析。 |
...
| 方法 | 签名 | 调用方 | 作用 |
|---|---|---|---|
| `place` | `place(o, dpi, resolve_panel) -> None` | `src/tavotto/app.py:1037` · `_export_produce_canvas` | 按对象类型落一个元素：panel（PDF 真矢量 show_pdf_page；位图带 crop/rotation 经 convert_to_pdf；opacity<1 或 flip 时按 dpi 位图嵌入）/ text / arrow / shape |
| `size_pt` | `size_pt -> (w_pt, h_pt)` | `src/tavotto/app.py:1045` · `_export_produce_canvas` | 样式检查报告里的页面尺寸 |
| `save_pdf` | `save_pdf(path) -> None` | `src/tavotto/app.py:1062` · `_export_produce_canvas` | 真矢量 PDF（deflate） |
| `save_tiff` | `save_tiff(path, dpi) -> dict` | `src/tavotto/app.py:1074` · `_export_produce_canvas` | 与 save_png 同一页同一次栅格化参数（ADR 0046）；编码器 tiffwrite.py |
| `save_png` | `save_png(path, dpi) -> None` | `src/tavotto/app.py:1076` · `_export_produce_canvas` | 同一页渲染 |
| `close` | `close() -> None` | `src/tavotto/app.py:1098` · `_export_produce_canvas` | finally 里关文档 |
