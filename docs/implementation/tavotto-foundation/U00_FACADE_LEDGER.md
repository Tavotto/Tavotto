...
| `render_preview_png` | `preview_cache` | public_contract | `src/tavotto/app.py:320` · `_write_render_cache` | `tests/test_render_cache.py:196` (user_contract)<br>`tests/test_mcp_normalize.py:285` (user_contract) | — | U07/U08：新栅格运行时（PDFium 候选）按同一宽度出 PNG；缓存层不动；跨 renderer 的抗锯齿差异按 03 §6「需要校准」记录阈值，不全局放大。 |
