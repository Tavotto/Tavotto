...
**Windows 上的替身与「宿主的包可见」**（#475 首次在 Windows 上跑事务用例：db5f994a 12 + 2 条 `managed_env_create_failed`、detail 空串 → 85b96a8f 让 detail 说得出来 → d0d331f7 打出真根因「挂了宿主 site-packages 仍 import 不到 matplotlib」）：POSIX 的替身是 exec 宿主解释器的 sh 包装，宿主的 site-packages 天然可见；Windows 的替身是 venvlauncher 副本 + pyvenv.cfg，它自己是个 venv，不带 `include-system-site-packages = true` 时宿主的包一个都看不见——`offline_managed_env` 问替身要 site-packages 挂进新代（`.pth`）拿到的是替身自己不存在的目录，新代 import 不到 matplotlib。修：`_windows_pyvenv_cfg` 开这一行，两种替身同一种语义；用例 `test_the_windows_standin_sees_the_hosts_site_packages_like_the_posix_one`（cfg 字面量 + 本机用真 `--system-site-packages` venv 验证「问解释器自己要 site-packages」的取法把 base_prefix 的 purelib 交出来、plain venv 看不见）。顺带留下的三件：① 产品侧 `managedenv._venv_built`——`-m venv` 之后让 venv 里的解释器自报一次 prefix，起不来就是没建成、detail 说清（`_venv_failure_detail` 覆盖空输出 / 退出 0 无文件），用例 `test_a_venv_whose_interpreter_does_not_start_is_not_built`，变异 M53 红；② `support.private_python.standin_base_probe()`：Windows 上按真依赖探一次（建 venv → 起 → pip），探不过的机器才让「用供应出来的解释器建受管代」的用例（transaction 12 条 + FO24 / FO26）skip-with-reason（`needs_real_base` 装饰器，装饰那一刻才探、模块 import 不起子进程——ccfad63e 在模块顶层求值曾让 Windows 两片收集期 NameError）；③ 探测与真实的一致性由 `test_the_windows_standin_base_probe_matches_reality` 看住，报告进 CI 日志；`test_the_support_module_imports_under_both_os_names_with_the_same_exports` 在子进程里把 os.name 钉成 nt / posix 各 import 一次。Windows 的这条真链另有 `private-python-targets.yml` 的 windows 腿用真 pbs 归档跑 TestRealChain（run 35603820985 绿）。
