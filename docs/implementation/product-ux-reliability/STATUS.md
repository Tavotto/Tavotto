...
| 命令 | 结果 |
| --- | --- |
| `PYTHONPATH=<wt>/src <repo>/.venv/bin/python -m pytest` | ✅ exit 0 —— **3463** passed / 34 skipped / 2 deselected / 0 failed，约 10 分（比 11 的 3370 +93：66 条导出用例 + 27 条参数化的错误码） |
| `cd web && pnpm test` | ✅ exit 0 —— **151** files / **1917** tests passed（比 11 的 147/1805 +4 文件 / +112 条） |
| `cd web && pnpm build` | ✅ exit 0（`tsc -b && vite build`） |
...
| `python scripts/build_mcp_widget.py` | ✅ 已重建（指纹 `628720e1c5a15125`，第五轮评审后）+ `--check` 通过 |
...
| **第五轮评审（PR #214，`c8479335`）** | ✅ 1 P1 + 2 P2 **全部成立、全部改**（两条 P2 都在十行以内，比开 Issue 便宜）；变异反证扩到**后端 25 / 前端 26，全红**。处置见 `TEST_MATRIX.md`「第五轮评审」 |
