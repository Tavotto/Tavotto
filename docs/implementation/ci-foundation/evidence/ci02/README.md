...
| `cache_seed_mutations.json` | 缓存种子后续 PR（分支 `ci/cache-seed-on-main`，2026-09-16）的变异反证 S01–S24，**24/24 KILLED**：种子的 if / Gate needs / shared-key 值与 os / profile 与命令 / cpython key 字符串与 os / pnpm os 与命令 / apt 包名与条件 / continue-on-error / fail-fast / 另一个 job 混进 push 或没有 if，每条记退出码与红的用例 | 会话 scratchpad 的 `mut/run.py`（同一套顺序：树干净 + 基线绿 → 目标串恰好 1 次 → 变异 → 清 `__pycache__` → pytest 退出码 → `git checkout --` → 核 md5） |
