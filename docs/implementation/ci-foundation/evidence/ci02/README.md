...
| `mutations_ci.json` | 合同测试的变异反证 M01–M21（ci.yml / package.json / tsconfig），**21/21 KILLED**，每条记目标串次数、退出码、红的用例、还原 md5 | `mutate_ci02.py`（会话 scratchpad，不进仓库；顺序：树干净 + 基线绿 → 目标串次数 == 期望 → 变异 → 清 `__pycache__` + `PYTHONDONTWRITEBYTECODE=1 -p no:cacheprovider` → pytest 退出码 → `git checkout --` → 核 md5） |
