...
| `compare.py` → `comparison.json` | before（CI00 29 个 merge_group 中位）与 after 逐 run 并排；`contended` 标记（关键路径上任一 job runner_wait ≥ 60s）；`qualification_model_zero_runner_wait`（runner_wait := 0 重放 DAG 的口径，不是测量）；感兴趣的 step 秒数（按 ci.yml `name:` 前缀取） | `python evidence/ci05/compare.py` |
