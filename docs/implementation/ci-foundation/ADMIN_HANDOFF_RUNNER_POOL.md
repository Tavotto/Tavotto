...
| C-1 … C-5 | **未批准**（随 B），留档 |
| D-1 … D-7 | **未批准**（随 B），留档 |
| F-1 … F-12 | `not_run`（已拍板，待管理员 + 两个后续 PR） |
...
| F-2 | ci-infra 的 `lab-qualification.yml`：`on: workflow_dispatch`（inputs `mode` / `sha` / `baseline_tag` / `use_prebuilt_dist` / `source_run_id`）。job ① `trust-check`（hosted）：checkout `Tavotto/Tavotto`（`fetch-depth: 0`）、`git merge-base --is-ancestor <sha> origin/main` 或 tag 指向——与今天 `lab-ci.yml::trust-check` 同一段 shell（**照抄，不是引用**：调用方的信任判断必须在调用方自己手里，`test_every_caller_gates_the_sha_through_a_trust_job` 的理由）；job ② `qualify`：`uses: Tavotto/Tavotto/.github/workflows/_lab-qualification.yml@main`（`uses:` 不接受表达式，钉不到被验的那个 SHA；定义取 main 上那份——main 已过 review；被验的**代码**仍是 `inputs.sha`，由 reusable 里的 checkout 取；公开仓库可读，不需要 token），`secrets: inherit`；job ③ `report`（hosted，`needs: qualify`，`if: always()`）：用 ci-infra 的 secret（fine-grained，只对 `Tavotto/Tavotto` 的 `statuses: write`）给那个 SHA 打一条 commit status（context `lab/<mode>`，state = qualify 的结论，target_url = 本 run），并上传汇总 artifact。reusable 仍是 `contents: read`，写权限只在 ci-infra 的 ③ 里 | ci-infra 仓库 + 两个 secret（`TAVOTTO_ACTIONS_READ`：取 `dist` artifact；`TAVOTTO_STATUS_WRITE`：回写 status——名字随便起，**值不进任何文件**） | 删 workflow | `gh workflow list -R Tavotto/ci-infra`；一次手动 dispatch（`mode=main`，sha = main 当前）在 ci-infra 有结论 run，且 `gh api repos/Tavotto/Tavotto/commits/<sha>/status` 里出现 `lab/main` |
| F-3 | 公开仓库 PR A（**先合**）：`_lab-qualification.yml` 的两处改动（F.0 第一行）；`lab-ci.yml::qualify` → `dispatch`；合同测试改主语（F.3）。**这个 PR 合入时 runner 还在公开仓库上**，所以 `lab-ci.yml` 的 dispatch 会打到 ci-infra、而 ci-infra 的 runner 还没注册——F-4 之前 lab-ci 会排队；顺序上 F-4 紧接着做 | `.github/workflows/` + `tests/` | `git revert` | 合同测试全绿；`actionlint` 0 |
...
| F-5 | 在 ci-infra 手动 dispatch 一次 `mode=nightly`（sha = main 当前）；再等一次 cron（或由 `lab-ci.yml` 的 push main 触发一次 dispatch）。**判据**：ci-infra 的 run 结论 success，`compat.json` / soak / benchmark 报告与迁移前那台机器上最近一次 nightly 的形状一致（同一台机器，`/srv/tavotto-ci` 的基线不变，benchmark 不该漂） | ci-infra run | — | `gh run list -R Tavotto/ci-infra --workflow lab-qualification.yml`；公开仓库 `gh api repos/Tavotto/Tavotto/commits/<sha>/status --jq '.statuses[] | select(.context | startswith("lab/"))'` |
| F-6 | 公开仓库 PR B：发布链切两段（F.2）+ 合同测试。**合入前用 `publish=false` 在 PR 分支上不能演练**（release.yml 只认 tag / dispatch 且 dispatch 要 main 祖先）——所以 PR B 合入后立刻做 F-7 | `.github/workflows/release.yml` + 新 `release-publish.yml` + `tests/test_release_workflow_contract.py` | `git revert` | 合同测试全绿 |
| F-7 | **发布演练**：`workflow_dispatch(ref=<main 当前 SHA>, publish=false)` → 第一段跑完并 dispatch ci-infra（`mode=release`, `use_prebuilt_dist=true`, `source_run_id=<第一段 run id>`）→ ci-infra 取到 `dist` artifact、`lab_release_gate` 绿、`report` 回写 `lab/release` status 并 dispatch `release-publish.yml(sha, source_run_id, lab_run_id, publish=false, ack_open_blockers=<原样>)` → 第二段 trust 重验 + `validate_artifacts` 全部产物真实存在并校验、最后不发布。**三个 run 都要有结论** | 三条 run | — | 第一段 `gh run view <id> --json conclusion`；ci-infra run success；第二段 run success 且 summary 里有「演练（publish=false）到此为止」 |
| F-8 | **注销公开仓库的 `tavotto-ci-01`**（F-5 与 F-7 都绿之后）：在它的实例目录 `sudo ./svc.sh stop && sudo ./svc.sh uninstall && ./config.sh remove --token <REMOVAL_TOKEN>`；删掉那把只给公开仓库用的 Deploy key（`self-hosted-runner.md` §9「停用 / 移除」） | lab VM 上公开仓库那个实例 | 重新注册（§7） | `gh api repos/Tavotto/Tavotto/actions/runners --jq .total_count` = **0**（-2/-3/-4 已在 A-2 注销）；ci-infra 的 runners 仍 online |
| F-9 | 公开仓库 PR C：`.github/actionlint.yaml` 与守卫 ② ④ 的主语按 F.3 收口；`docs/ci/self-hosted-runner.md` §7 的注册命令改成 ci-infra 的 URL、§1 把「备用形态」改成「现行形态」；`docs/ci/release-qualification.md` 加两段链路 | 文档 + 测试 | `git revert` | `pytest tests/test_merge_queue_workflows.py tests/test_release_workflow_contract.py tests/test_docs_references.py` 0 |
| F-10 | 一次 A-3 的反向实测（可选，安全）：临时分支 PR 里一个 `runs-on: [self-hosted, tavotto-lab]` 的空 job（只 `echo`）→ 预期 **永远 queued**（公开仓库没有这个标签的 runner）→ 关 PR。这是 CI04 §2.2 那个洞关掉的直接证据；把 run 号记进 `evidence/ci04/` | 临时 PR | 关 PR | `gh api …/runs/<id>/jobs --jq '.jobs[].status'` = queued；关 PR 后 run 被 cancel |
| F-11 | 更新 `admin_inventory.json`：runner 注册层级（仓库级 → 私有仓库级）、实例数（1）、A-1 / A-3 标「被 F 绕开」 | 文档 | — | JSON 解析 |
| F-12 | `runner_pool_ready` 保持 `not_run`（这不是池）；CIP-024「PR 无法通过修改 runs-on 接触可信 lab」在 F-8 + F-10 之后可以改 pass——证据 = 公开仓库 runners `total_count` 0 + F-10 的 queued run | `acceptance.json` | — | — |

### F.2 发布链切两段（PR B 的设计；不实施，只写清）
...
- **谁能 dispatch 第二段**：只有持 `TAVOTTO_ACTIONS_WRITE`（ci-infra 的 secret，fine-grained，只对 `Tavotto/Tavotto` 的 `actions: write`）的 ci-infra `report` job；但第二段**不信任「谁触发」**——它重验 SHA、重验 blocker、重读 `lab/release` status（一次 API 调用，不是循环）并要求 state = success 且 `target_url` 指向 `lab_run_id` 那个 run。手工 dispatch 第二段而 lab 没绿，就停在 trust2。
...
| 用例 | 今天钉的 | 迁移后钉什么 |
|---|---|---|
| `test_qualification_is_defined_exactly_once` | `lab-ci.yml::qualify` 与 `release.yml::lab_release_gate` 都 `uses` reusable、不带自己的 steps | reusable 仍唯一；公开仓库里**没有**调用方；`lab-ci.yml::dispatch` 与 `release.yml::dispatch_lab` 的命令是 `gh workflow run lab-qualification.yml -R Tavotto/ci-infra`，参数 `sha` 来自 trust 的输出 |
| `test_every_caller_gates_the_sha_through_a_trust_job` | 每个 `uses` reusable 的 job 的 `sha:` 来自带 `--is-ancestor` 的 job | 每个 **dispatch** job 的 `-f sha=` 来自带 `--is-ancestor` 的 job（主语从「调用方」换成「派发方」）；ci-infra 那边的 trust-check 由 ci-infra 自己的合同测试钉（私有仓库要有自己的 `tests/`，把这条用例抄过去） |
| `test_the_release_gate_cannot_be_evicted_by_a_routine_lab_run` | reusable 的 `concurrency.group` 含 `github.workflow` | ci-infra 的 `lab-qualification.yml` 一个 workflow 服务两种档位——槽名改成含 `inputs.mode`（`lab-qualification-${{ inputs.mode }}`），release 与 nightly 不共槽；用例的主语跟着改到 reusable 的 group 表达式 |
| `TestRunnerTrustZones` ② | `tavotto-lab` 只在 reusable，调用方 == {lab-ci.yml, release.yml}，事件 ⊆ {push, schedule, workflow_dispatch} | `tavotto-lab` 只在 reusable；公开仓库**没有任何 workflow** `uses` 它（集合为空）；reusable 的事件仍 == {workflow_call} |
