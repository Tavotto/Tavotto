...
| F-5 | 在 ci-infra 手动 dispatch 一次 `mode=nightly`（sha = main 当前）；再等一次 cron（或由 `lab-ci.yml` 的 push main 触发一次 dispatch）。**判据**：ci-infra 的 run 结论 success，`compat.json` / soak / benchmark 报告与迁移前那台机器上最近一次 nightly 的形状一致（同一台机器，`/srv/tavotto-ci` 的基线不变，benchmark 不该漂）。**首验（2026-09-16，PR A 合入 `1879201b` 触发的 `mode=main`）：链路机械全通**——公开 lab-ci `trust-check` / `dispatch` 绿 → ci-infra run 35112349056 的 `qualify` 真跑在 `tavotto-lab-01` 上 → `report` 把 `lab/main = failure` 写回 `1879201b`；**红在两处 ambient `GITHUB_REPOSITORY`（PR C 修）**：① 「常规测试套件」唯一失败 `tests/test_distribution_metrics.py::test_missing_token_fails_loudly_instead_of_silently_skipping`（`assert 0 == 2`——用例没钉 `GITHUB_REPOSITORY`，采集器看到 `Tavotto/ci-infra` 按「fork 没配 secret」退 0）；② 同族 `scripts/ci/upgrade_acceptance.py::REPO_SLUG` 会去 ci-infra 找 N-1 发行档（这次没跑到）。修法：reusable 的 `qualify` job env `TAVOTTO_SOURCE_REPOSITORY: Tavotto/Tavotto` + `_common.source_repository()` 唯一出口 + 用例钉主语（合同 `tests/test_lab_source_repository.py`）。PR C 合入后再跑一次 nightly 才算 F-5 完成 | ci-infra run | — | `gh run list -R Tavotto/ci-infra --workflow lab-qualification.yml`；公开仓库 `gh api repos/Tavotto/Tavotto/commits/<sha>/status --jq '.statuses[] | select(.context | startswith("lab/"))'` |
...
| F-8 | **注销公开仓库的 `tavotto-ci-01`**（F-5 与 F-7 都绿之后）：在它的实例目录 `sudo ./svc.sh stop && sudo ./svc.sh uninstall && ./config.sh remove --token <REMOVAL_TOKEN>`（两个实例同名，按目录 / 服务名认，`self-hosted-runner.md` §9「停用 / 移除」的表）。**Deploy key 不删**：它是这台机器 clone `Tavotto/Tavotto` 的唯一通路（§5，github.com HTTPS 不可达），ci-infra 实例里 reusable 的 checkout 也靠它 | lab VM 上公开仓库那个实例 | 重新注册（§7） | `gh api repos/Tavotto/Tavotto/actions/runners --jq .total_count` = **0**（-2/-3/-4 已在 A-2 注销）；ci-infra 的 runners 仍 online |
