...
| C-1 … C-5 | **未批准**（随 B），留档 |
| D-1 … D-7 | **未批准**（随 B），留档 |
| F-1 … F-12 | F-3 **PR A 已开**（分支 `ci/lab-runner-private-repo-a`）；其余 `not_run`（已拍板，待管理员 + PR B / PR C） |
...
| F-6 | 公开仓库 PR B：发布链切两段（F.2）+ 合同测试。**PR 已开（分支 `ci/lab-runner-private-repo-b`，叠在 PR A 上）**；ci-infra 侧配套在分支 `report-dispatches-release-publish`（`report` 加派发第二段那一步），两边同一天落地——**先合 ci-infra 再合 PR B**：反过来 PR B 合入后第一次发版 lab 绿了也没人派第二段。**合入前用 `publish=false` 在 PR 分支上不能演练**（release.yml 只认 tag / dispatch 且 dispatch 要 main 祖先）——所以 PR B 合入后立刻做 F-7 | `.github/workflows/release.yml` + 新 `release-publish.yml` + `tests/test_release_workflow_contract.py` 等四个测试文件 + ci-infra `lab-qualification.yml::report` | `git revert`（两边各一个） | 合同测试全绿；`actionlint` 0；变异 49 + 13 条全部打红 |
| F-7 | **发布演练**：`workflow_dispatch(ref=<main 当前 SHA>, publish=false)` → 第一段跑完并 dispatch ci-infra（`mode=release`, `use_prebuilt_dist=true`, `source_run_id=<第一段 run id>`）→ ci-infra 取到 `dist` artifact、`qualify` 绿、`report` 回写 `lab/release` status 并 dispatch `release-publish.yml(sha, source_run_id, lab_run_id, publish=false, ack_open_blockers=<原样>)` → 第二段 trust2 重验 + `validate_artifacts` 全部产物真实存在并校验、最后不发布。**三个 run 都要有结论**。演练绿之后、第一次正式 tag 之前还有一件手工事：**PyPI trusted publisher 的 Workflow name 从 `release.yml` 改成 `release-publish.yml`**（OIDC 的 `workflow_ref` 是实际跑 pypi-publish 的那份；演练测不到这一步，`docs/RELEASING.md`「PyPI 发布」） | 三条 run + PyPI 发布者配置 | — | 第一段 `gh run view <id> --json conclusion`；ci-infra run success 且 summary 有「第二段已派发」；第二段 run success 且 summary 里有「演练（publish=false）到此为止」 |
...
- **实施时发现的一条边界（PR B）**：第一段的 dispatch 输入 `pypi`（none / testpypi / pypi）没有随两跳载荷传到第二段——接口钉的是五项。第二段 publish=true 只可能来自 tag，所以 `pypi` job 的门是 `publish && vars.PYPI_PUBLISH_ENABLED`（从前 tag 触发那条路本来就只看它），**TestPyPI 通道在两段链里不存在**；`release.yml` 的 `pypi` 输入留着不改 `on:` 的形状，但已无消费者。要恢复得把它加进两跳载荷（ci-infra 接口 +1）。另：`n1_update_windows` 随 `github_release` 一起搬到第二段（它 needs github_release）。
