...
* **依赖闭包 wheel 表**（`scripts/dev/u10_wheel_matrix.py`，`evidence/u10/wheel_matrix.json`）：五个原生包 ×
  Python 3.10–3.14 × 四个正式目标（win x64 / macOS arm64 / macOS x86_64 / Linux x86_64）全部有 wheel；informational：
  Linux aarch64 全有。**合入 main 时（2026-09-24）的更正**：第一版这里写的是「macOS x86_64：pikepdf 10.x 五档全部没有
  wheel」，**那是假阴性**——脚本给 Intel 的平台标签只列到 `macosx_14_0`，而 pikepdf 10.x 的 x86_64 wheel 标的是
  `macosx_15_0`；Apple Silicon 那组一次给了 11–15，拿到 `macosx_14_0` 的 wheel 也记「有」。两个方向都没量「要求多高」。
  真实情况：**pikepdf 10.13 要求 macOS 14（arm64）/ 15（x86_64）**，pypdfium2 5.13 要求 13，其余 ≤ 11。于是：
  ① 桌面版的最低系统随之抬到 **14.0 / 15.0**（见 §2 表；用户 2026-09-24 拍板「如实抬」）；② main 上此时已有 Intel
  桌面版（ADR 0076），第一版据假阴性写的「Intel 不支持 / pip 需自编 qpdf」**作废**，Intel 桌面版保留；③ 脚本改成把标签
...
| **桌面版最低 macOS** | 11.0（PyMuPDF 的 wheel 要求 ≤ 11） | **Apple Silicon 14.0 / Intel 15.0**——打进去的 pikepdf 10.x wheel 要求的下限（pypdfium2 要求 13）；更老的系统上 dmg 打不开，而不是打开后渲染失败 | `support-matrix.json` 的 `min_os`；`tests/test_support_matrix.py` |
...
| `tauri.conf.json` 的 `minimumSystemVersion` 改回 11.0 / 矩阵 `min_os` 低于 wheel 表的 `macos_floor` / 钉的 pikepdf 换了版本而 wheel 表没重生成 | `test_macos_min_os_covers_the_bundled_wheels`、`test_wheel_floor_evidence_is_for_the_pinned_versions` |
| Intel 腿不叠 `tauri.intel.conf.json` / 叠的条件漏了 `runner.os`（Windows 腿的 arch 也是 x86_64） | `test_intel_leg_layers_its_own_minimum_and_checks_the_real_app` |
...
* **最低 macOS 抬到 14 / 15**（§1 更正、§2 表）——已拍板（2026-09-24）：如实抬，不从源码自编低 deployment target 的
  qpdf / pikepdf、不降回 pikepdf 9.x。`desktop-tauri.yml` 只在发版链上跑，叠 Intel 配置与 `--expect-min-os` 两步在 PR 上
  **not_run**，第一次真跑是下一次发版（发布链上「每一步第一次执行都可能失败」的那一类风险：发版前先手动触发一次 desktop-tauri）。
