...
| 版本探针 / bridge 导入闭包（RC-087） | 桥新增 import `artifactinspect` → `BRIDGE_IMPORTS_AT_MIN` 同步；它晚于 v0.15.0，**下次发版 `MIN_TAVOTTO_VERSION` 必须抬到那一版**（现在不能写一个还没发的号：`test_plugin_manifest_min_tavotto_version_is_real`） | `tests/test_codex_plugin.py::test_min_tavotto_version_is_reestimated_when_the_bridge_imports_change` |
