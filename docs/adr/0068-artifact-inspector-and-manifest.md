...
* MCP 入口只有 standard 政策：`tavotto_export` 没有 `inspection` 参数（严格政策走 HTTP 的 `inspection` 段）；要给模型开
  严格档时在 `bridge.export` 加参数、经同一份 `inspection_profile` 解析即可，不另写阈值。

## 5. 入口（U08 第三切片：HTTP / SSE / MCP / 前端 / Playground 的审计结论）

**判据只有一份**：每个入口都经 `engine/artifactinspect.inspect_produced` 拿到同一形状的 `manifest`，前端对它的解读只有
`web/src/lib/artifactInspection.inspectionState()` 一处；「未核验」在每一处都不是「已通过」。

| 入口 | 做了什么 | 看护 |
|---|---|---|
| HTTP 同步 `/api/export` / 异步 `/api/export/start` + SSE `export.progress` / `state` | B 已接 `inspect=`；`outputs[].manifest` 随 `job.to_payload()` 走 SSE 与补拉两条路（同一份投影）；阶段 `inspecting` 有两种语言的进度文案（后端会发出的每个阶段名 ↔ `dialogs.json export.phase` 同源判据） | `tests/test_export_inspection.py`、`tests/test_export_phase_labels.py` |
| MCP `tavotto_export` | `bridge.export` 经 `engine_exportjob.run(..., inspect=)` 接**同一份**接线（`backend="worker"`，契约层 `probe_asset` 按需 import）；位图 `Produced` 带期望像素（图幅 × dpi，与 `artifactcheck` 同一换算）让 `size` 量得到；回执 `files[].manifest` 原样带出，旧键一个不少；给模型的文字加一行「产物核验」，三组各自点名（未通过 / 已核验 / 未核验），一组都不省；坏文件 `artifact_rejected` 进 `partial`、不发布 | `tests/test_mcp_export_inspection.py`（独立读取器：stdlib `pdfread` + hashlib；替身 worker 写的是合法最小文件 `tests/support/artifactbytes.py`） |
| 版本探针 / bridge 导入闭包（RC-087） | 桥新增 import `artifactinspect` → 三处同源一起改：`BRIDGE_IMPORTS_AT_MIN`、`server.py` 的 `_BRIDGE_IMPORT` 探测串（resolver 判老引擎够不够用；漏了 = 交棒后桥 ImportError 崩死，U09 合并态全量抓到过一次）、桥本身；它晚于 v0.15.0，**下次发版 `MIN_TAVOTTO_VERSION` 必须抬到那一版**（现在不能写一个还没发的号：`test_plugin_manifest_min_tavotto_version_is_real`） | `tests/test_codex_plugin.py::test_min_tavotto_version_is_reestimated_when_the_bridge_imports_change`、`tests/test_mcp_resolver.py::test_bridge_import_probe_matches_the_bridge` |
| 前端导出对话框 | `ExportOutput.manifest`（可选，老服务端没有 = 未核验）；每件产出一行：全部可判项 `verified` 才画绿勾「已核验」，有 `unknown` 按名字列「未核验：…」（中性色、无勾），有 `failed` 红着列「核验未通过：…」（standard 下文件已交付，用户投出去之前得知道），`not_applicable` 不画；高级选项里「严格核验产物」开关 → 请求多一段 `inspection: {mode: "strict", profile_id}`（不勾时载荷逐字节不变，快照指纹不含它） | `web/src/lib/artifactInspection.test.ts`、`exportRequest.test.ts`、`ExportDialog.test.tsx`（「产物核验」组） |
| Codex 内嵌画布（MCP 传输） | 「已导出」提示后按文件名点名失败 / 未核验的文件（`inspectionRollup`）；提示是中性色不是绿 | `artifactInspection.test.ts` |
| `/api/render` 的 503（候选后端的背压） | `<img>` 看不见状态码：`web/src/lib/imgRetry.useRetryingSrc` 对 `/api/render` 地址按 1 / 2 / 4 s 有界重试（cache-bust `r=n`），blob / data / `/api/file` 失败不重试；画布面板、缩略图、版本 / 图库 / 导出对话框的缩略图共用 | `web/src/lib/imgRetry.test.tsx` |
| 四类 Web 构建 / Playground（RC-089） | 审计结论：SPA（浏览器 / 桌面壳同一份，`apiUrl` 相对根）、MCP 画布（`vite.mcp.config.ts`，MCP 传输、`panelSrc` 有 HTTP 时才给地址）、Playground（`vite.playground.config.ts` `base: './'`，`panelSrc: () => null`、**没有导出面板**、不发任何 `/api/*` 请求）——本切片没有改任何资源路径；Playground 里不存在能把「未核验」画成绿的组件 | `web/src/playground/browserEngineTransport.test.ts`、`tests/test_playground_build.py`（既有） |
| 旧响应结构（RC-090） | `files[] / export_dir / warnings / vector` 一字不改；新增只有 `manifest` 键 | `test_mcp_export_inspection.py`（旧键逐个点名）、parity 里 `test_export_legacy_items_texts_contract` |
