...
## 反证（每条变异一次就红；脚本在会话 scratchpad，结果 18/18）
...
单块不用 zlib 头、预览没有快路、快路不看成品在不在、warm 不交给 prewarm、prewarm 什么都不做——各自红在上表的用例上。

两条中途被抓出来的**假绿**（修掉之后才算上面的 18/18）：「算值前后指纹相同」这条判据**不可观测**——算的过程中被改写的，指纹
