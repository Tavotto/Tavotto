...
**第 2 档只算用户自己指定的**（2026-09-21，#469）：源码模式下 `bootstrap.install()`
也把自建 venv 写进同一个 `worker.python` 键，但那是自动决策——它作为
`managed_venv` 候选留在第 4 档（自身之后、系统链之前），不压过第 3 档，也不算
「全局固定」。判据唯一出处 `pool.explicit_worker_python()`。
