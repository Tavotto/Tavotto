...
* **全局显式解释器生效时不提供安装目标。** ADR 0018 §四把 `TAVOTTO_WORKER_PYTHON`
  ...
  `create_plan()` / `install()`（后者在环境租约里）都先问 `pool.explicit_worker_python()`，
  生效就一个目标都不给、计划不形成、已形成的也不执行；全局解释器的改动经
  `envlease.unless_mutating()` 与租约互斥。改发 `dependency_interpreter_pinned`
  + `pinned:{python, source, variable}`，界面给「恢复自动检测」（清全局设置）或
  「清掉环境变量 <variable> 后重启」。优先级本身不动；唯一的澄清是源码模式下
  `bootstrap.install()` 写进 config 的自建 venv **不属于第 2 档**——它是自动决策，
  改为老链条里的一档候选（自身之后、系统链之前），不再压过项目级环境。
