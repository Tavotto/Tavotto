...
| 预览 | `/api/render` 在候选下走 `rendercore.preview.PreviewCache`（`facade.preview_cache(CACHE_DIR)`：键 = 内容身份 + 后端 build + PDFium 版本 + 字体政策 + 像素参数；内容身份从 PreviewCache 自己抄进缓存目录的那份源字节上算——键与渲染绑同一份字节（U07），所以不再从 `source_sha1` 的 (mtime, size) memo 注入；缓存目录变了就重建实例）；child 有界队列满 → **503 + Retry-After: 1**（背压不是故障），别的 child 错误 → 500 带稳定 code | `test_api_render_under_the_candidate_serves_a_png_of_the_bucket_width_from_the_cache`、`…reports_backpressure_as_503_and_other_failures_as_500`、`test_switching_the_backend_changes_the_preview_cache_key` |
