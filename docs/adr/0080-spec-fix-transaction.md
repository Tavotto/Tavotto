...
| native 图（`tavotto run`） | **暂不支持自动修复**（2026-09-25 用户决定，叠栈 PR 维持）。端点在任何一次渲染之前回 409 `specfix_native_unsupported`，那张 live 图一个字节都不碰；前端问题照常列出，修复按钮不可用、悬停说一句为什么（`isNativePanelIssue`，判据与面板角标同一个出处 `runtimeAssetStore.profile`，未知按 safe），批量里它们计为 `native_unsupported`。理由：native 进程归用户（ADR 0021），不能作废重起，而事务回滚不干净时的兜底恰恰是作废 worker。下一行的「与文档不一致」机制补的是**任何渲染**（不只是修复）留下的不一致；修复事务是多次渲染，要放开还得让整个事务在会话锁里成一段，未做 |
