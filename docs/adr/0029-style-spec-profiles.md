...
## 6a. 修订（2026-09-24）：默认样式的派生规则、样式里数字的单位

用户实测「应用默认样式」之后：字号层级被抹平（所有文字角色一律 9 pt）、数据线一律变成最细的
0.5 pt、字体只换到标题与轴标题、缩小放进拼版的图套完样式检查照样红。§4「规范说正文 9 pt /
拉丁 Times New Roman / 线宽第一档，样式照它生成」那句因此改成下面三条（仍然**只从规范派生、
不写第二份数字**，实现 `profilestore._builtin_style_record`）：

- **字号保留层级**：标题 / 轴标题 / 图内文字 = `default_font_size_pt`；刻度与图例 = 规范允许的
  **最小一档**（0.5 pt 格子上、严格高于 `absolute_min_font_size_pt`、不低于
  `min_effective_font_size_pt`、不高于正文），图例再夹进 `legend_policy` 区间。默认规范下是
  9 / 9 / 8.5 / 8.5——写成 8 的话正好踩在 `eff <= floor` 那条不含等号的边上。
- **字体覆盖所有带 `fontfamily` 的文字角色**：`ticks`（ADR 0051 的引擎入口）与 `legend_text`
  （图例容器没有这个字段）。图例标题没有 override 入口，应用时如实进 `unmappable`。
- **数据曲线的线宽不统一**：档位是「允许哪几档」不是「该用哪一档」；偏离档位的线由预检报、
  一键修复吸到**最近**的档位，保留各条线之间的相对粗细。边框（结构）仍统一到边框档位第一档。

**样式里以 pt 计的数字是页面上读者量到的 pt**（与预检的 `panelScale` 同一个口径）：写入 ÷ 缩放比、
提取 × 缩放比（`lib/stylePresets.PAGE_PT_PROPS` / `toScriptValue` / `toPageValue`）。画布标注的
`sizePt` 本来就是页面 pt。应用样式的界面入口是左栏「样式」面板；设置 › 样式页只管库。

> 同日再修订：「应用样式」改成**画布绑定一套样式并跟随它**，见 [ADR 0081](0081-canvas-follows-style.md)。
> 同日第三次修订：属性页也按页面 pt 显示与输入，换算收成 `stylePresets.pagePtLens` 一处、样式面板与属性页共用，
> 见 [ADR 0082](0082-inspector-shows-page-pt.md)。
