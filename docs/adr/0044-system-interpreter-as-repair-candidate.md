...
> 2026-09-23 修订（[ADR 0079](0079-user-environment-discovery-and-auto-adoption.md) §四）：**跑前的依赖门**里，
> 用户自己的环境装齐了脚本要的包、且此刻的解释器是机器替用户挑的，就直接改用（通知轨说出口、可「改回」，
> 改回之后不再自动挑）。本节仍约束运行后（`missing_dependency`）那条修复卡片。
