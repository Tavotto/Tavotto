    ...
    *,
    ...
    try:
        ...
        if not ok:
            ...
            elif job.trace.failed_phase is None and job.report is not None:
                # 图全好、只有报告坏了（写不出 / 没有载荷）：坏在哪一步也要说——不然轨迹末尾是
                # `report ok`、`failed_phase=null`，而作业明明是 partial（Codex #498 P2）
                job.trace.fail("report", job.report.error_code or "report_failed")
        else:
