...
| `axestraversal.ordered_axes`、`spinemodel.py`、`tickmodel.py`、`_cls_key`、能力表、3D / 箭头 / 散点 marker | axes 遍历、特殊 artist 与 artist family 能力层 → `axes-and-artist-families.md` | `fig.axes` 只许出现在白名单函数里；族模块（`axestraversal` / `spinemodel` / `tickmodel`）是叶子、只迁移一份、`HANDLERS` 只经展开登记；编号顺序 `fig.axes` → 子 axes → 寄生轴是契约；能力按真实 getter 实况判不按类名（映射中的 Collection 不给 facecolor）；认不出的 Artist 只开 visible / zorder；annotate 的箭头绝不出端点 | `tests/test_axes_traversal_authority.py`、`tests/test_engine_family_modules.py`、`test_parasite_axes.py`、`test_artist_families.py`、`test_worker_roundtrip.py` |
