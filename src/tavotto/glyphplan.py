...
* `₂`（U+2082）在拉丁段，旧后端的 base-14 没有它，于是 PyMuPDF 自己悄悄换了一张
  `Noto Serif Regular` 把它画出来。画面是对的，**但没有任何人知道刚才换了脸**——
  用户选的是 sans-serif，那个下标是衬线的。（U10 起主脸 Liberation 自带它，ADR 0072。）
* `━`（U+2501）一类制表符在拉丁段，拉丁脸没有，而我们手里的 CJK 脸恰好有——
  码位判据不会去问它，结果是一个本可以画出来的方框。
...
## 四层，顺序不可交换
    ...
    primary   请求的那个族的脸自己画得出（批准字体集合里的 Liberation 三族；表取 12 张脸的交集）
    cjk       中日韩脸画得出（**只有码位在 CJK 段、或前两层都没有时**才轮到它）
    fallback  前两层都没有、但渲染器自己挑得出一张脸——U10 起**恒空**（没有隐式回退脸，ADR 0060 §1）；
              层名保留是让四步顺序与两侧的闭集常量不变
    missing   谁都画不出——导出上就是一个方框，必须进问题系统（文本层里字还在）

顺序里那句「只有码位在 CJK 段」是刻意的：把它去掉，`₂` 会从主脸改判到中日韩脸，
**既有文档的像素会变**。本模块的分层与改造前逐字符等价，只多救回第 4 步那 87 个码位（实测）。

## 覆盖判据从哪来

Python 侧问**真字体**（`rendercore/facade.py` 经批准字体注册表提供 oracle），所以导出那一端永远
