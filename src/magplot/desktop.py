"""桌面 sidecar 模式：Tauri 壳的受控后端（`magplot --desktop-sidecar`）。
...
- 一次性启动 nonce → 短生命周期 HttpOnly 会话 cookie 的桌面认证：
  ...
  `ps eww` 就能看到别的进程的 env，管道不行），`MAGPLOT_DESKTOP_NONCE`
  只作为调试用的回退，读到后立即从 `os.environ` 摘除，绝不落盘、绝不进日志。
- 握手文件（`MAGPLOT_DESKTOP_HANDSHAKE` 指定路径，tmp+replace 原子写）：
...
浏览器模式下这些钩子全部旁路（`MAGPLOT_DESKTOP_STATE` 未设置即直接放行），
bootstrap 端点回 404——普通 `magplot` CLI 的行为一个字节都不变。
"""
...
LOG = logging.getLogger("mm.desktop")

COOKIE_NAME = "magplot_desktop"
...
def install(flask_app) -> None:
    ...
    `MAGPLOT_DESKTOP_STATE` 不存在，钩子直接放行、端点 404，行为零变化。
    """

    @flask_app.post(BOOTSTRAP_PATH)
    def _desktop_bootstrap():
        state: DesktopState | None = flask_app.config.get("MAGPLOT_DESKTOP_STATE")
        if state is None:
...
def read_launch_credentials(stdin=None, environ=None) -> tuple[str | None, int | None, object]:
    ...
    nonce = env.pop("MAGPLOT_DESKTOP_NONCE", None)
    parent_raw = env.pop("MAGPLOT_DESKTOP_PARENT_PID", None)
...
class SidecarServer:
    ...
    def __init__(self, flask_app, state: DesktopState,
        ...
        flask_app.config["MAGPLOT_DESKTOP_MODE"] = True
        flask_app.config["MAGPLOT_DESKTOP_STATE"] = state
    ...
    def _cleanup(self) -> None:
        ...
        if self._handshake is not None:
            try:
                ...
                pass
        self._app.config.pop("MAGPLOT_DESKTOP_STATE", None)
        self._app.config.pop("MAGPLOT_DESKTOP_MODE", None)
