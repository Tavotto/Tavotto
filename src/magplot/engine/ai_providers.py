...
`~/.codex/config.toml`。** Magplot 只是借用户已装好的 CLI 干一件小事（改图
...
# codex 侧临时覆盖用的 provider id 与密钥环境变量名。用 magplot 前缀是为了
# 不和用户自己 config.toml 里的 provider 撞名（`-c` 只在本次进程生效，但
# 同名会遮蔽用户的定义，日志里也会看不明白）。
CODEX_PROVIDER_ID = "magplot"
CODEX_KEY_ENV = "MAGPLOT_CODEX_API_KEY"
