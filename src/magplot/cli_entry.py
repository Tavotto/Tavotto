"""`magplot` 命令与 `python -m magplot` 的入口。**纯标准库，不 import app。**

为什么不直接指向 `magplot.app:main`：`open` / `doctor` 这两个子命令是给外部
...
`magplot.app:main` —— 同一条命令在两种安装形态下行为不同：

* 插件的交接会连着调两次 `magplot open`，每次都白付一整个应用的冷启动；
* `doctor` 本该是「装坏了怎么查」的那把工具，可只要某个界面依赖 import 失败
  （缺 DLL、装了一半、pymupdf 的 wheel 与解释器不匹配），它自己也起不来——
  最需要它的时候正好用不了。

分派放在 argparse **之前**：主入口是纯 flag 形态（`magplot --figures …`），
