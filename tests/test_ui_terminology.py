...
**判据的主语**：前端两种语言的全部语言包（八个命名空间、每一个叶子值，含命令面板的
搜索关键词）。桌面壳自己那份（`src-tauri/src/i18n.rs`，原生菜单文案不经语言包）**不在
这里扫**——它由 `tests/test_desktop_i18n.py` 的 `test_menu_uses_the_four_ui_nouns` 看护
（#663），两处各扫一份会成为同一条判据的两个出处。**不看** key 名
...
#: (来源, 位置) → 理由。来源是 `web:<语言>`；位置是 `命名空间:点分 key`。**只收指帮助手册 / 官方说明的「文档」**。
