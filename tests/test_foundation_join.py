...
#: 回执里 pid 核过的两种写法：直核 `ok`；Windows 上 venv 的 python.exe 是 launcher、解释器是它的子进程 → `ok_via_launcher`
PID_CHECKED = ("ok", "ok_via_launcher")
...
def _build_project_venv(proj: Path, tmp_path: Path) -> dict:
    ...
    if proc.returncode == 2 and "夹具前提不成立" in proc.stderr:
        # 构造脚本自己核出「项目 Python 与应用 Python 同一个 minor」：这是夹具前提没摆对（换一个
        # --python），不是产品红——skip 并写 not_run；CI 的具名任务把 skip 当红（两层防线各守各的：
        # workflow 那层管解释器别撞、这层管前提不满足时说清楚而不是报一条像产品缺陷的红）。
        pytest.skip(f"not_run: 项目与应用同 minor，换 --python（{proc.stderr.strip()[-300:]}）")
...
def test_fo32_existing_env_open_edit_replay_export_through_the_new_rendercore(tmp_path):
    ...
    with fa.running_app(proj, tmp_path / "work", env_overrides=env) as app:
        ...
        assert receipt["pid_check"] in PID_CHECKED
        ...
        assert facts["completeness"] == "complete" and facts["pid_check"] in PID_CHECKED
...
class TestManagedEnvJoin:
    ...
    def test_fo32_managed_env_private_python_joint_install_then_rendercore(self, client, tmp_path):
        ...
        pp = managed["private_python"]
        assert pp["id"] == self.source.id
        # 私有 Python 两种就位态都是这条出口（ADR 0063 §九之二）：这台机器还没有它 → `required=True`
        # （缓存有归档就 0 字节、不联网；没有就要下载）；同一台机器已经被别的项目 / 同一 workflow 前一步供应过 →
        # `required=False`、零字节、不联网——环境本身仍要授权、仍要建代。两种都得把来源说出口，不许沉默。
        present = privatepython.python_of(self.source) is not None
        assert pp["required"] is (not present), pp
        assert pp["network_required"] is (not pp["cached"]), pp
        if pp["cached"]:
            assert pp["download_bytes"] == 0, pp
        else:
            assert pp["download_bytes"] == self.source.size, pp
        evidence.append(
            f"asked workdir (ambiguous_data) then dependencies (private python {self.source.id}: "
            f"{'already present' if present else 'to provision'}, cached={pp['cached']}, "
            f"download_bytes={pp['download_bytes']}, network_required={pp['network_required']})"
        )
        # ③ 一次授权：供应（或已就位）→ 代事务装 h5py（+ adapter）→ done。计划上的 `private_python` 段只在
        # 「这次授权包含供应」时有值（已就位 → None，与 U05 的两项目同机用例同一形状）；门上的披露在 ② 已核过。
        plan, rec = self._authorize(client, "scripts/figure_h5.py")
        if present:
            assert plan["private_python"] is None, plan["private_python"]
        else:
            assert plan["private_python"]["required"] is True, plan["private_python"]
        ...
        rt = receipt["runtime"]
        assert receipt["completeness"] == "complete" and receipt["pid_check"] in PID_CHECKED
...
def _rewrite_same_size(path: Path, text: str) -> None:
    ...
    `text` 用 `\n` 写；落盘沿用原文件的换行形态——Windows 上 checkout 会把夹具的 CSV 转成 CRLF（每行多一个
    字节），按 LF 算「同样的字节数」在那里恒不成立（#498 第三轮 Windows 腿：28 ≠ 33）。以字节写、不走文本层。
    """
    before = path.stat()
    original = path.read_bytes()
    newline = b"\r\n" if b"\r\n" in original else b"\n"
    payload = text.encode("utf-8").replace(b"\n", newline)
    assert len(payload) == before.st_size, f"要同样的字节数：{len(payload)} != {before.st_size}"
    assert payload != original, "改了却没变"
