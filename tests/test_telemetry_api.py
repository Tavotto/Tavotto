...
def project(tmp_path):
    ...
    (figs / "tavotto_registry.json").write_text(json.dumps({
        "scripts": {"fig1.py": {"entry": "main", "cost": "light",
                                "stems": ["Fig1_保密项目"]}},
...
def test_patch_settings_round_trip(client, monkeypatch):
    ...
    body = client.patch("/api/telemetry/settings",
                        json={"consent": "enabled", "source": "first_run"}).get_json()
    assert body["consent"] == "enabled" and body["enabled"] is True

    body = client.patch("/api/telemetry/settings",
                        json={"consent": "disabled"}).get_json()
    assert body["consent"] == "disabled" and body["enabled"] is False
    telemetry.reset_for_tests()


def test_patch_settings_rejects_garbage(client):
    assert client.patch("/api/telemetry/settings",
...
def test_event_endpoint_accepts_allowlisted_event(client, telemetry_sent):
    resp = client.post("/api/telemetry/event", json={
        "event": "figure_opened",
        "properties": {"asset_kind": "pdf", "editable": True}})
...
def test_event_endpoint_rejects_anything_off_the_allowlist(client, telemetry_sent,
                                                           payload):
...
# AI：最敏感的一条边界
# ---------------------------------------------------------------------------
SECRET_PROMPT = ("把 /Users/me/secret/论文数据.csv 里的第三条曲线改成红色，"
                 "顺便用我的 API key sk-abcdef0123456789")


def _run_ai(client, agent="codex", **extra):
    return client.post("/api/ai/run", json={
        "agent": agent,
        "id": "Fig1_保密项目.pdf",
...
def test_ai_invocation_never_transmits_the_prompt(client, project, telemetry_sent,
                                                  monkeypatch):
    ...
    for forbidden in (SECRET_PROMPT, "sk-abcdef0123456789", "论文数据.csv",
                      "/Users/me/secret", "Fig1_保密项目", "fig1.py",
                      "axes_0.lines_2", "第三条曲线", "投稿版 Figure 3",
                      "sess-123", "element"):
        assert forbidden not in blob, f"{forbidden} 泄漏进了遥测事件"


def test_ai_agent_name_is_an_enum_not_free_text(client, project, telemetry_sent,
                                                monkeypatch):
...
def test_ai_start_survives_telemetry_failure(client, project, telemetry_sent,
                                             monkeypatch):
