...
def test_two_platforms_land_in_one_manifest(tmp_path):
    ...
    _artifact(tmp_path / "desktop-tauri-dmg", "Magplot.app.tar.gz", "MAC-SIG\n")
    _artifact(tmp_path / "desktop-tauri-nsis", "Magplot_0.7.0_x64-setup.nsis.zip", "WIN-SIG")

    man = mod.build_manifest(tmp_path, "0.7.0", "v0.7.0", "erwanjun", "magplot", "更新说明")
    ...
    assert man["platforms"]["windows-x86_64"]["url"] == (
        "https://github.com/erwanjun/magplot/releases/download/v0.7.0/"
        "Magplot_0.7.0_x64-setup.nsis.zip"
    )


def test_missing_signature_is_a_hard_error(tmp_path):
    """有包没签名 = 装到一半才发现对不上。宁可不发清单。"""
    _artifact(tmp_path, "Magplot.app.tar.gz", sig=None)
...
def test_installer_and_dmg_are_not_mistaken_for_update_packages(tmp_path):
    ...
    _artifact(tmp_path, "Magplot-0.7.0-macOS.dmg")
    _artifact(tmp_path, "Magplot-0.7.0-Windows-Setup.exe")
    with pytest.raises(SystemExit, match="一个更新包都没找到"):
        mod.build_manifest(tmp_path, "0.7.0", "v0.7.0", "erwanjun", "magplot", "")
...
def test_intel_mac_gets_no_entry(tmp_path):
    ...
    比「查不到更新」糟糕得多。
    """
    _artifact(tmp_path, "Magplot.app.tar.gz")
...
def test_cli_writes_the_file(tmp_path):
    _artifact(tmp_path / "in", "Magplot.app.tar.gz")
