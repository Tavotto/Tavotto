...
| `lib/readinessText.ts`、`store/projectReadinessStore.ts`、`RegistryDialog`、左栏偏好、`WorkdirConfirmDialog` / `WorkdirRow`、`envStore.setWorkdirMode`、`DependencyPrepareDialog` / `depRepairStore.prepare` / `adoptUserEnvironment`、`lib/userEnvironmentText.ts`、通知轨「已改用你的环境」 | 接入状态与左侧外壳 → `readiness-and-left-shell.md` | 句子只有 `statusLabel()` / `reasonText()`（读 `reason_code`）；`allEditable` 一档不逐张重复；「没测量」三档不压扁；界面不执行动作、冲突不预选；首开的运行目录确认框只翻译后端的三档载荷、歧义不预选、「运行」一次 PATCH、「稍后」留在错误块；跑前的依赖授权框只翻译整份联合计划、先绑定计划再只发 plan_id、blocked 摆理由不装、「不准备直接运行」是明确的 skip；装齐的用户环境排在安装目标前、只交 id 采用，后端已自动改用时不弹框、通知轨给「改回」（ADR 0079）；侧栏「偏好」与「此刻开着」是两件事、自动让位绝不写回偏好 | `store/projectReadinessStore.test.ts`、`RegistryDialog.test.tsx`、`ProjectReadinessBanner.test.tsx`、`WorkdirConfirmDialog.test.tsx`、`WorkdirRow.test.tsx`、`DependencyPrepareDialog.test.tsx`、`notificationRail.test.tsx`、`canvas/drawerViewportResize.test.tsx`、`store/uiStore.test.ts` |
