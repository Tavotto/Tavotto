...
| `web/src/perf/`、`PerfProbeHud`、`interactionStore.begin/end`、热路径上的 `perfSpan` / `perfInput` / `perfCount` | 拖动性能探针 → `perf-probe.md` | 热路径只 import `perf/core.ts`（叶子）、没录制时零成本；插桩点闭集且与 `scripts/perf_report.py` 同步；片段边界只在 `interactionStore`；自动测试一律 `pointercancel` 收尾、绝不改文档；报告只有数字；判断在分析器不在产品 | `perf/core.test.ts`、`perf/synthetic.test.ts`、`e2e/perf-probe.spec.ts`、`tests/test_perf_probe.py` |
