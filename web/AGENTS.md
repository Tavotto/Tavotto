...
| `lib/typography.ts`、`typographyAdapter.ts`、`TypographyControls.tsx`、`lib/glyphPlan.ts`、图内 pt 量的写入器 | 属性能力层与 Typography 控件 → `typography-capability-layer.md` | 不许按对象类型 switch 写属性；值四档不压扁；`propertyPathOf` 唯一；图内 pt 量按页面值进出、换算只在 `pagePtLens`（ADR 0082） | `lib/pagePtLens.test.ts`、`inspector/pagePtInspector.test.tsx`、`lib/typography.test.ts`、`typographyAdapter.test.tsx`、`lib/canvasTextFont.test.ts`、`tests/test_typography_families.py` |
...
| `lib/pathGeom.ts`、`lib/shapeGeometry.ts`、`pickElement` / `pickElementStack`、⌥ 轮换、`patchContents`、图内拖动吸附 `inFigureSnapCandidates`、随行元素 `companionPatchesFor`、图内平移 `InFigureMove`、方向键微调 `canvas/nudge.ts` | 命中与选择几何 → `hit-and-selection-geometry.md` | 填充按 nonzero、距离换到 mm；⌥ 只换选中且必须说出口；拖动与方向键共用一份 `InFigureMove`（ADR 0093） | `canvas/arrowNudge.test.tsx`、`e2e/arrow-nudge.spec.ts`、`canvas/patchCarryDrag.test.tsx`、`lib/pathGeom.test.ts`、`elementPathSelection.test.tsx`、`shapeOutline.test.tsx`、`canvas/twinAxesPick.test.tsx`、`e2e/twin-axes-pick.spec.ts`、`canvas/inFigureDrag.test.tsx`、`canvas/axesCompanionDrag.test.tsx` |
| `lib/session.ts`、`types/document.ts`、`documentStore` 的 `loadSeq` / `derivedSeq` / `registerTxnFinalizer`、`lib/autosave/diskWriter.ts`、`panelSourceSync.ts`、`assetStore`、`lib/projectDocs.ts` | 项目、文档模型与自动保存 → `project-document-and-autosave.md` | schema 3 只在持久化边界；外部派生唯一写入口 `applyDerivedUpdate()`；手势中不许写 | `store/documentStore.test.ts`、`lib/overrideIdentity.test.ts`、`store/layoutAutoReflow.test.ts`、`store/panelSourceSync.test.ts`、`hooks/useEngineSync.test.ts`、`lib/projectDocs.test.ts`、`e2e/cross-tab-paste.spec.ts`、`lib/autosave/diskWriter.test.ts` |
