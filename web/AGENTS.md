...
| `web/src/i18n/`、`UiMessage`、`errors.json`、`backendErrorText()`、`lib/stylePresets.ts` / `specBinding.ts` / `exportDefaults.ts` | 多语言 → `i18n.md` | 资源静态 import；code 一旦发布不能改名；`pnpm i18n:check` 是硬门禁；界面名词只有项目 / 排版 / 画布 / 项目包 | `web/scripts/i18n-check.mjs`、`i18n/overflow.test.tsx`、`e2e/i18n.spec.ts`、`preflight.golden.test.ts`、`tests/test_i18n_dead_keys.py`、`tests/test_error_codes.py`、`tests/test_style_schema_pair.py`、`tests/test_ui_terminology.py` |
