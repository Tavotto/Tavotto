...
| `web/src/perf/`、`PerfProbeHud`、`interactionStore.begin/end`、热路径上的 `perfSpan` / `perfInput` / `perfCount` | 拖动性能探针 → `perf-probe.md` | 热路径只 import `perf/core.ts`（叶子）、没录制时零成本；插桩点闭集且与 `scripts/perf_report.py` 同步；片段边界只在 `interactionStore`；自动测试一律 `pointercancel` 收尾、绝不改文档；报告只有数字、桌面版存不上就说存不上；判断在分析器不在产品 | `perf/core.test.ts`、`perf/synthetic.test.ts`、`perf/probeStore.test.tsx`、`e2e/perf-probe.spec.ts`、`tests/test_perf_probe.py` |
...
| `lib/readinessText.ts`、`store/projectReadinessStore.ts`、`RegistryDialog`、左栏偏好、`ElementTree` 的行、`WorkdirConfirmDialog` / `WorkdirRow`、`envStore.setWorkdirMode`、`DependencyPrepareDialog` / `depRepairStore.prepare` / `adoptUserEnvironment`、`lib/userEnvironmentText.ts`、通知轨「已改用你的环境」 | 接入状态与左侧外壳 → `readiness-and-left-shell.md` | 句子只有 `statusLabel()` / `reasonText()`（读 `reason_code`）；`allEditable` 一档不逐张重复；「没测量」三档不压扁；界面不执行动作、冲突不预选；首开的运行目录确认框只翻译后端的三档载荷、歧义不预选、「运行」一次 PATCH、「稍后」留在错误块；跑前的依赖授权框只翻译整份联合计划、先绑定计划再只发 plan_id、blocked 摆理由不装、「不准备直接运行」是明确的 skip；装齐的用户环境排在安装目标前、只交 id 采用，后端已自动改用时不弹框、通知轨给「改回」（ADR 0079）；侧栏「偏好」与「此刻开着」是两件事、自动让位绝不写回偏好；元素树行 memo、props 只收显示字段（不收 `panel` / `el`）、回调树级稳定 | `store/projectReadinessStore.test.ts`、`RegistryDialog.test.tsx`、`ProjectReadinessBanner.test.tsx`、`WorkdirConfirmDialog.test.tsx`、`WorkdirRow.test.tsx`、`DependencyPrepareDialog.test.tsx`、`notificationRail.test.tsx`、`canvas/drawerViewportResize.test.tsx`、`store/uiStore.test.ts`、`components/left/elementTreeRerender.test.tsx` |
| `lib/exportRequest.ts`、`store/exportStore.ts`、`lib/exportFigures.ts`、`lib/exportName.ts`、`lib/artifactInspection.ts`、`lib/imgRetry.ts`、导出对话框 | 统一导出管线 → `export-pipeline.md` | 载荷只由 `buildExportRequest()` 构造；`scope=original` 没有 x/y/w/h（类型上就没有）；PPI 无位图时 `null`；作业活在 store、SSE 外加轮询；「导出期间被编辑过」用此刻文档重算指纹；文件名规则严格同源对、不用 `String.trim()`；「哪一张」只在 `exportFigures.ts` 判；产物核验的解读只有 `inspectionState()` 一处（全部可判项 verified 才画绿、没有 manifest = 未核验）；严格核验只在勾了时带 `inspection` 段；`/api/render` 失败按 `imgRetry` 有界重试；`manifest.identity / provenance` 与作业 `trace`（U09）只是类型上接住，界面不画、不解读 | `lib/exportRequest.test.ts`、`store/exportStore.test.ts`、`lib/exportFigures.test.ts`、`lib/artifactInspection.test.ts`、`lib/imgRetry.test.tsx`、`components/ExportDialog.test.tsx`、`tests/golden/filename_vectors.json`、`tests/test_export_request.py`、`tests/test_export_phase_labels.py` |
