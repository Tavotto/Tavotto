...
| 任何读 manifest 几何后写文档的动作、`panelDisplayView`、override upsert | 显示回退 ≠ 几何权威 → `display-fallback-vs-geometry-authority.md` | 显示 API 与权威 API 分开；没有视觉位移就不写 override | `store/geometryAuthority.test.ts`、`store/alignAction.test.ts`、`canvas/alignUndoConvergence.test.tsx`、`test/renderFixtures.ts` 的 `seedExactRender()` |
...
| `store/svgPreviewStore.ts`、`lib/svgStyle.ts`、`trackPointer`、`useFieldGesture` | 假实时预览：预览平面与历史平面严格分开 → `fake-realtime-preview.md` | 临时 transform 从 base 现算；`pointercancel` 与 `pointerup` 分开；样式预览与 `applyStyleEdit` 共用 `styleTargets` | `lib/svgStyle.test.ts`、`store/svgPreviewStore.test.ts`、`canvas/fakeRealtimeDrag.test.tsx`、`e2e/fake-realtime.spec.ts` |
...
| `lib/legendModel.ts`、`LegendCard`、`LegendSpacingCard`、`setLegendPlacement` | 图例条目与绑定 → `legend-entries-and-binding.md` | 脱开判据 = 任一 `handle_*` override 在；`LEGEND_ENTRY_STYLE_PROPS` / `LEGEND_BINDINGS` 与引擎严格同源 | `inspector/legendCard.test.tsx`、`legendSpacingCard.test.tsx`、`controls/pickers.test.tsx`、`tests/test_legend_binding.py` |
...
| `lib/pathGeom.ts`、`lib/shapeGeometry.ts`、`pickElement` / `pickElementStack`、⌥ 轮换、`patchContents` | 命中与选择几何 → `hit-and-selection-geometry.md` | 填充按 nonzero、距离换到 mm；⌥ 只换选中且必须说出口 | `canvas/patchCarryDrag.test.tsx`、`lib/pathGeom.test.ts`、`elementPathSelection.test.tsx`、`shapeOutline.test.tsx`、`canvas/twinAxesPick.test.tsx`、`e2e/twin-axes-pick.spec.ts` |
...
| 画布标签图层、`CanvasThumb`、标注与 `lib/shapeSwitch.ts`、混排对齐、`writeBackAnnotations.ts`、`EmptyState`、`viewportStore.fitted` | 画布对象、标注与工作区视口 → `canvas-objects-and-workspace.md` | 剪贴板主路径是原生 ClipboardEvent；类型切换一次 commit 不换 id；前后端几何公式同源 | `shapeSwitch.golden.test.ts` + `tests/test_compose_switched_shapes.py`、`canvas/shapeOutline.test.tsx`、`store/workspace.test.ts`、`e2e/cross-tab-paste.spec.ts`、`e2e/nav-audit.spec.ts` |
| `AssetBrowser`、`ScriptLibrary`、`scriptRunStore` / `runtimeAssetStore` / `scriptLibraryStore` / `assetStore` / `depRepairStore` / `envStore`、`FigurePickerDialog` | 素材库普通入口 → `asset-library.md` | 六个 store 都有项目代际；同脚本防并发、取消等原请求以 `execution_cancelled` 落地；多 Figure 结果绝不只显示第一张；runtime 卡没有假值；「编辑原图」必然加进文档并说出口、「添加到画布」一律 `addFigureToLayout`；`role="option"` 里不嵌可 Tab 控件；TIFF 经 `panelSrc` 走 `/api/render` | `scriptRunStore.test.ts`、`ScriptLibrary.test.tsx`、`AssetBrowser.runtime.test.tsx`、`AssetBrowser.tiff.test.tsx`、`lib/panelSrc.test.ts`、`projectSwitchAssets.test.ts`、`openRequest.test.ts`、`FigurePickerDialog.test.tsx`、`e2e/asset-library.spec.ts`、`projectSwitchDepRepair.test.ts` |
| `NativeConfirmDialog`、`nativeSessionStore`、`nativePanelState`、`?native=` / `tavotto:open` 的 `native` | `tavotto run` 的桌面面 → `tavotto-run-desktop-side.md` | 前端只提交不透明 `native_id`；确认屏是闸；未知不等于 native | `nativeSessionStore.test.ts`、`NativeConfirmDialog.test.tsx`、`openRequest.test.ts`、`tests/native/test_run_beta_claims.py` |
| `lib/readinessText.ts`、`store/projectReadinessStore.ts`、`RegistryDialog`、左栏偏好、`ElementTree` 的行、`WorkdirConfirmDialog` / `WorkdirRow`、`envStore.setWorkdirMode`、`DependencyPrepareDialog` / `depRepairStore.prepare` / `adoptUserEnvironment`、`lib/userEnvironmentText.ts`、通知轨「已改用你的环境」 | 接入状态与左侧外壳 → `readiness-and-left-shell.md` | 句子只有 `statusLabel()` / `reasonText()`；界面不执行动作、冲突不预选；自动让位绝不写回偏好 | `store/projectReadinessStore.test.ts`、`RegistryDialog.test.tsx`、`ProjectReadinessBanner.test.tsx`、`WorkdirConfirmDialog.test.tsx`、`WorkdirRow.test.tsx`、`DependencyPrepareDialog.test.tsx`、`notificationRail.test.tsx`、`canvas/drawerViewportResize.test.tsx`、`store/uiStore.test.ts`、`components/left/elementTreeRerender.test.tsx`、`components/left/workspaceList.test.tsx`、`store/projectSwitchSerial.test.ts` |
