...
| `canvas/context-bar/`、`actions.beginCrop`、`store/arrangeStore`、`arrangeButtons.ts` | 多选浮动栏与共享排列参照 → `multi-selection-context-bar.md` | 进裁剪一律 `beginCrop`；浮动栏只发意图；参照只有 `arrangeStore` | `context-bar/position.test.ts`、`multiSelectionBar.test.tsx`、`context-bar/elementMultiBar.test.tsx`、`e2e/multi-selection-bar.spec.ts`、`store/alignSelectedTo.test.ts`、`store/arrangeStore.test.ts`、`canvas/contextBar.test.tsx` |
