...
| `lib/readinessText.ts`、`store/projectReadinessStore.ts`、`RegistryDialog`、左栏偏好、`WorkdirConfirmDialog` / `WorkdirRow`、`envStore.setWorkdirMode` | 接入状态与左侧外壳 → `readiness-and-left-shell.md` | 句子只有 `statusLabel()` / `reasonText()`（读 `reason_code`）；`allEditable` 一档不逐张重复；「没测量」三档不压扁；界面不执行动作、冲突不预选；首开的运行目录确认框只翻译后端的三档载荷、歧义不预选、「运行」一次 PATCH、「稍后」留在错误块；侧栏「偏好」与「此刻开着」是两件事、自动让位绝不写回偏好 | `store/projectReadinessStore.test.ts`、`RegistryDialog.test.tsx`、`ProjectReadinessBanner.test.tsx`、`WorkdirConfirmDialog.test.tsx`、`WorkdirRow.test.tsx`、`canvas/drawerViewportResize.test.tsx`、`store/uiStore.test.ts` |
