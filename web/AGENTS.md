...
| `store/svgPreviewStore.ts`、`lib/svgStyle.ts`、`trackPointer`、`useFieldGesture` | 假实时预览：预览平面与历史平面严格分开 → `fake-realtime-preview.md` | 临时 transform 从 base 现算；`pointercancel` 与 `pointerup` 分开；样式预览与 `applyStyleEdit` 共用 `styleTargets` | `lib/svgStyle.test.ts`、`store/svgPreviewStore.test.ts`、`canvas/fakeRealtimeDrag.test.tsx`、`e2e/fake-realtime.spec.ts`、`canvas/renderSwapFeel.test.tsx` |
