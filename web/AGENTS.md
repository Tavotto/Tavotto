...
| `lib/desktop.ts`、`hooks/menuActions.ts`、`lib/updateNotice.ts`、`UpdateNoticeDialog` | 桌面感知与更新 → `desktop-and-update.md` | 组件不得直接 import `@tauri-apps/*`；只查一条更新通道；菜单转发与键盘同一个让位判据 | `lib/desktop.test.ts`、`hooks/menuActions.test.tsx`、`components/UpdateNoticeDialog.test.tsx`、`store/updateStore.test.ts` |
