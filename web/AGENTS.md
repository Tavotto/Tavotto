...
边框只给区域边界、选择状态与浮层；可编辑框是 `field` 凹面 + 内阴影（聚焦才浮回白底 + accent 边）。
**持久表面里只有「真的是一张卡」的东西有投影（`--shadow-card`：素材卡 / 会话卡 / 任务行 / 诊断卡），
浮层用 `--shadow-pop` / `--shadow-dialog`**（宪法第二十二节，2026-09-15）。
...
**可编辑框只有一副**（`ui/fieldBox.ts`：`field` 凹面 + `shadow-field`，聚焦白底 + accent 边；TextInput / NumberField /
Select / SearchInput / `inspector/controls/PickerTrigger` 共用；批次 2，形态 2026-09-15 改凹面）；带样张的取值选择器一律 `Popover + PickerTrigger + OptionGrid`
