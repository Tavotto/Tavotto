...
export default interface Resources {
  "ai": {
    ...
    "panel": {
      ...
      "emptyHint": "助手会改图的脚本。改完可查看差异、回滚。",
      "noCli": "没有可用的编码 Agent。",
      "noPanelHint": "带 { } 标记的面板来自脚本，助手可直接修改。",
  ...
  "dialogs": {
    "closeGuard": {
      ...
      "discard": "不保存",
      "save": "保存并关闭",
      "saveFailed": "无法保存，窗口保持打开。请处理保存问题，或选择「不保存」直接关闭。",
      "title": "有未保存的修改"
    },
    "export": {
      "advanced": "高级选项",
      ...
      "jobLost": "找不到这次导出，服务可能重启过。文件是否写出不确定，查看导出目录或重试。",
      "locate": "定位",
      ...
      "replaced": "已覆盖",
      "reportTitle": "随成图写一份 JSON：用的规范、字体字号线宽画幅、检查结果与导出设置。投稿留档用。",
      ...
      "sizeDiskDiffers": "磁盘上的原件是 {{dw}} × {{dh}} mm（脚本保存时裁到了内容范围）。按图幅 {{w}} × {{h}} mm 出图。",
    ...
    "layout": {
      "conflict": "磁盘上已有一份名为「{{name}}」的文档，不是本窗口写的。",
    ...
    "mcp": {
      ...
      "splashNoHost": "此画布要在支持 MCP Apps 的 Codex 里打开。没有 UI 的 host 里，同一套 tavotto_* 工具也能打开、改图、预检、导出。",
    ...
    "nativeRun": {
      "approve": "运行并连接",
      "approving": "正在连接…",
      "argCount_other": "{{count}} 个（参数值不经过此窗口）",
      "cancel": "取消",
      ...
      "loading": "正在读取运行请求…",
      "permissionNotice": "使用项目自己的 Python。脚本的权限与在终端里运行时相同，Tavotto 只接管其中的 Matplotlib 图。只运行你信任的代码。",
    ...
    "onboarding": {
      ...
      "steps": {
        ...
        "export_original": {
          ...
          "scope": {
            "body": "「原图」按图自己的尺寸输出，不使用画布缩放。确认后关闭面板继续。",
            "title": "看看原图导出"
          },
          "title": "看看原图导出"
        },
        ...
        "welcome": {
          "body": "这是离线教程副本：随便改，随时重开，不会碰你的项目。接下来用真实操作走两条核心流程。",
          "title": "用示例了解 Tavotto"
        }
      },
    ...
    "playground": {
      ...
      "integrityNote": "浏览器工作区里真正执行的文件的 SHA-256，与你交出来的原文逐字节比对。它查的是脚本或引擎意外改动了源文件；蓄意规避这项检查的脚本不在此列——它与你的图跑在同一个解释器里。",
      "integrityTitle": "完整性",
      "integrityUnavailableNote": "浏览器算不出哈希，比对没有执行——所以这里不说「未改动」。",
      ...
      "taskContinue": "继续探索",
      "taskDoneBody": "图已改变。{{filename}} 一个字也没动。",
      "taskDoneTitle": "完成。",
      "taskDoneUnverified": "图已改变。浏览器无法核对源文件哈希，不下「未改动」的结论。",
    ...
    "profiles": {
      ...
      "clearField": "清除{{field}}",
      "conflict": "「{{name}}」已被改过。请重新打开后再改。",
      "copyOf": "{{name}} 副本",
      "delete": "删除",
      "deleteBody": "用过它的项目不受影响，各自留有副本。",
      "deleteTitle": "删除「{{name}}」？",
      "detail": {
        "id": "标识",
        "origin": "来源",
      ...
      "restore": "恢复默认值",
      "restoreBody": "内容会回到它复制来源的那份，改动会丢失。",
      "restoreConfirm": "恢复",
      "restoreNeedsOrigin": "不是从内置复制来的，没有可恢复的默认值",
      "restoreTitle": "把「{{name}}」恢复成默认值？",
      "save": "保存",
      "saved": "已保存「{{name}}」",
      "snapshotHint": "项目保存选中时的规则快照。全局同名配置后来改了也不影响，除非打开「跟随更新」。",
    ...
    "readiness": {
      ...
      "orphanConflicts": "不止一个脚本声称生成这些图名：{{stems}}。项目里还没有对应的图文件，可在下方「全部脚本」里指定。",
      ...
      "agents": {
        ...
        "codexInstall": {
          ...
          "error": {
            ...
            "doctor": {
              "interpreter_unusable": "插件的启动命令在这台机器上跑不起来，Codex 里会没有工具。这次只是诊断，没有改动；装一次会换成验证能跑的解释器。",
            ...
            "interpreter_unusable": "插件的启动命令在这台机器上跑不起来，Codex 里会没有工具。Windows 上的 python3 常是微软商店的占位命令，看着存在，一跑就退。安装会换成验证能跑的解释器；换不成，就装一个 Python，或用 TAVOTTO_MCP_PYTHON 指一个再试。",
  ...
  "errors": {
    "backend": {
      ...
      "no_figures_captured": "脚本跑完了，却没有产生图：没有 savefig，也没有留下 pyplot Figure。展开输出看看——多半是数据文件没找到，或分析提前结束。",
      ...
      "non_finite_on_disk": "磁盘上的文档含 NaN 或 ∞，不是合法 JSON，浏览器无法读取。Tavotto 未改动该文件；请检查是哪个外部工具写坏了它。",
      ...
      "replay_divergence": "写回被阻止：热会话与全量重放不一致。请把此信息报告给开发者。",
      ...
      "worker_build_timeout": "脚本没能跑完，工作进程已重启，请重试。Tavotto 按输出判断卡死：20 分钟没有任何输出，或连续输出满 4 小时，才会中断。展开输出看它停在哪一步；本来就要静默很久的，在那一段里打一行进度。",
    ...
    "preflight": {
      ...
      "bitmapEmbed": "翻转或半透明的面板按导出 DPI 位图嵌入 PDF，矢量文字不保留。",
      ...
      "fontBelowFloor": "图内文字的有效字号 {{effective}} pt 未超过绝对下限 {{floor}} pt。",
      ...
      "paletteLineMarkers": "{{count}} 条曲线都没有 marker，黑白打印或色觉障碍读者难以区分。可考虑加 marker 或用不同线型。",
      "paletteSemantic": "色谱 {{cmap}} 不在推荐色系里（按 sequential / diverging / categorical 语义选：{{url}}）。",
      "panelTextNotVerifiable": "矢量面板还没有引擎 manifest（未渲染或非脚本产物），图内文字字号与字体无法自动核验。",
      "rasterDpi": "位图面板的等效分辨率 {{dpi}}dpi 低于规范的 {{min}}dpi",
      "rasterTextNotVerifiable": "位图面板没有矢量文字层，字号无法核验。请确认最终字号大于规范下限。",
      "renderError": "面板最近一次渲染失败，导出前请先修复",
      "spinesNotEnclosed": "坐标轴缺 {{sides}} 边。规范要求封闭坐标轴。",
      "staleRender": "面板脚本已更新，还没重建。导出的是旧图。",
      ...
      "unappliedOverride": "面板有 {{count}} 条图内修改还没应用到渲染，成图会与画布不一致。"
    },
    "problems": {
      "ariaLocate": "定位到这个对象",
    ...
    "writeBackAnnotations": {
      "flipped": "面板已翻转，无法映射标注坐标",
      "rasterOnly": "素材只有位图，没有矢量 PDF",
      "rotated": "面板已旋转，无法映射标注坐标"
    }
  },
  "inspector": {
    "aiRunning": "有任务在运行",
    ...
    "arrange": {
      ...
      "columns": "列",
      "copyStyle": "复制样式",
      "copyStyleTip": "复制最后选中对象的样式：面板取裁剪、旋转、不透明度；文字取字号、字重、颜色、对齐。",
    ...
    "control": {
      ...
      "legendCustomHint": "图例已移到自定义位置。点任一预设（含「最佳位置」）即可复位。",
      "legendOutside": {
        "bottomCenter": "下方居中",
        "leftCenter": "左侧中",
        "rightBottom": "右侧下",
    ...
    "element": {
      "absentAppearance": "图上可见，但这一版无法在这里编辑——它来自脚本。",
      "advanced": "高级",
      "alignBaseline": "基准",
      "alignBaselineSize": "等宽/等高基准",
      "alignHdist": "水平等距（≥3）",
      "alignHint": "子图改的是它在图中的占比，文字与图例改的是锚点，都不影响画布上的面板尺寸。",
      ...
      "colorScaleTip": "色图与色阶上下限是同一份状态：改一边，另一边跟着变。",
      "editElement": "修改图内元素",
      "editProp": "修改{{label}}",
      ...
      "hint": {
        "scatterSize": "pt² 是面积，不是直径。数值翻倍，点约宽 1.4 倍。"
      },
      "howAiBody": "直接改写 .py 脚本文件。每次运行前自动留一份快照，任务历史里可以「回滚此次修改」。",
      "howAiTitle": "改图助手的修改",
      "howBothBody": "脚本改动后，指向已不存在元素的图内修改会被忽略，并在问题面板中提示。其余照常生效。",
      "howBothTitle": "两者相遇时",
    ...
    "legend": {
      ...
      "followHint": "示意线由图中对象派生；断开后才能单独改它的颜色、线型和标记。",
      "hide": "隐藏",
    ...
    "panel": {
      ...
      "bitmapBoth": "翻转和不透明度都会让面板在导出 PDF 里按位图嵌入。矢量文字不再可选中。",
      "bitmapFlip": "翻转的面板在导出 PDF 里按导出 DPI 嵌入为位图。矢量文字不再可选中。",
      "bitmapOpacity": "不透明度低于 100% 时，面板在 PDF 中导出为高分辨率位图。矢量文字不再可选中。",
      "building": "正在构建…",
      ...
      "ptHintBad": "原图 {{base}} pt × 缩放 {{scale}}%，低于 6 pt 出版会看不清",
      "replace": "替换素材",
      "replaceAction": "选择…",
      ...
      "source": "源文件",
      "sourceAdvanced": "源文件与高级",
      "sourceHint": "写回会覆盖原始 PDF/PNG 文件，但会留下备份，可恢复。",
    ...
    "writeBack": {
      ...
      "annotationsTitle": "压在面板上的画布箭头、文字、形状会以矢量按当前位置画进原 PDF，PNG 同步重出。写回后从画布移除，可撤销。",
      ...
      "restoreBody": "恢复：在面板「历史」里回到任一版本，或从备份目录拷回。生成图的脚本不会改动。",
      "rewriting": "正在重新渲染…",
      "scriptChanged": "生成脚本 {{script}} 已改动，当前仍是旧代码。请重新渲染该面板，确认后再写回。",
      "sizeMismatch": "写回后的页面尺寸与重放结果对不上。文件已替换，原件在备份目录，请取回并报告给开发者。",
      "sourceChanged": "素材 {{file}} 已在外部改动。刷新素材面板后再写回。",
  ...
  "project": {
    ...
    "figurePicker": {
      ...
      "empty": "脚本还没有登记任何图。到素材库的「脚本」区运行它。",
      "listAria": "脚本产出的图",
      "needsRun": "先运行一次",
      "runtimeBadge": "运行时图",
      "title": "选择一张图——{{script}}"
    ...
    "handoff": {
      ...
      "panelMissing": "没找到面板 {{stem}}——脚本可能还没产出，或它在别的图库里",
    ...
    "picker": {
      ...
      "tutorialBadge": "教程项目",
      "tutorialHint": "离线示例项目，随便改、随时重来，不会碰你自己的项目。",
      "tutorialLabel": "新手教程",
      "tutorialOpening": "准备中…",
  ...
  "workspace": {
    ...
    "assets": {
      ...
      "retry": "重试",
      "runtimeBadge": "运行时图",
      "runtimeCardTitle": "{{stem}}\n来自 {{script}}\n单击选中 · Enter 编辑原图 · Shift+Enter 添加到画布 · 空格看大图",
      "runtimeFromScript": "来自脚本 {{script}}",
      "runtimeNeedsRun": "尚未运行，运行后才有预览",
      "runtimeNoFile": "此图来自脚本运行，没有对应的原始图文件。你仍可编辑、组图和导出，导出会创建新文件。",
    ...
    "confirm": {
      ...
      "replaceAssetBody_other": "此面板有 {{count}} 项图内修改，绑定在原脚本的元素上，换素材后会清空（可撤销）。位置、尺寸、裁剪与层级保留。",
      "replaceAssetConfirm": "替换并清空修改",
      "replaceAssetTitle": "替换为「{{name}}」？",
      "resetOverridesBodyBaked": "这些修改已写回原始文件。恢复后，此面板回到源脚本当前生成的状态（可撤销），原始文件与源脚本保持现状，同一文件的其他面板不受影响。",
      "resetOverridesBody_other": "此面板会回到源脚本当前生成的状态。{{count}} 项图内修改将清除（可撤销），源脚本与原始文件不变，同一文件的其他面板不受影响。",
    ...
    "crash": {
      "blank": "打开空白文档",
      "body": "文档已自动保存在本机，刷新后从最后一次快照继续。若刷新后再次出错，改用「打开空白文档」；文档不会删除，可从「最近文档」取回。",
      "title": "界面出错了"
    },
    "docBanner": {
      "conflictDisk": "磁盘上：{{canvases}} 张画布 · {{objects}} 个对象 · 改动于 {{time}}",
      "conflictDiskUnknown": "无法读取磁盘上的内容摘要",
      "conflictExternal": "磁盘上的文档已被 Tavotto 之外的改动覆盖，本窗口的编辑没有写入。",
    ...
    "nativeSession": {
      ...
      "detach": "放手",
      "resume": "继续运行脚本",
      "resumeHint": "先把图恢复成脚本留下的样子，再继续。你的编辑会在下次停下时重放。",
      "state": {
    ...
    "status": {
      ...
      "scriptChanged_other": "脚本已更新，{{count}} 个面板已重新渲染",
      "sourceLinked_other": "已找到源脚本，双击面板即可编辑图内元素（{{count}} 个面板）",
      "sourceLostEditing": "源脚本关系已失效，已返回画布。图片和排版没有删除。",
      "sourceLost_other": "源脚本关系已失效，{{count}} 个面板已改为普通图片。图片和排版没有删除。",
