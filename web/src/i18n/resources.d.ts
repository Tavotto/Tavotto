...
export default interface Resources {
  ...
  "dialogs": {
    ...
    "export": {
      ...
      "inspection": {
        ...
        "failedItems": "核验未通过：{{items}}",
        "unknown": "未核验",
        "unknownItems": "未核验：{{items}}",
        "verified": "已核验"
      },
      ...
      "start": "开始导出",
      "strictTitle": "按所选规范重新打开写好的文件核对：完整性、尺寸、字体嵌入、文字层、图像分辨率。有一项不合格或无法核验，那个文件就不会保存。EPS 不在核验范围内。",
      "strictToggle": "严格核验产物",
    ...
    "layout": {
      ...
      "saveTitle": "另存为文档",
      "saveToProject": "存进项目",
      "saveToProjectHint": "这份排版还只在本机。存进项目之后，再保存会直接更新项目里的这个文件。",
      "saveToProjectTitle": "存进项目",
      "saved": "已另存为文档：{{name}}",
      "savesIntoLabel": "保存到",
      "saving": "正在保存…"
    },
    "mcp": {
      ...
      "exported": "已导出 {{files}}",
      "inspectionFailed": "（{{files}} 核验未通过）",
      "inspectionUnknown": "（{{files}} 未核验）",
      "issuesTitle": "出版规范预检",
      ...
      "splashFetchFailed": "取回会话状态失败。重新打开这张图；仍然如此请连同下面的诊断一起报告。",
      "splashFetching": "正在从 Tavotto 取回会话状态…",
      "splashNoHost": "此画布需在支持 MCP Apps 的 Codex 中打开。没有界面的宿主里，同一套 tavotto_* 工具同样可以打开、编辑、预检和导出。",
      "splashStripped": "宿主交给画布的工具结果里没有会话（structuredContent 为空）。多半是结果超过了宿主的体积上限被清空——升级 Tavotto 插件后重新打开这张图；仍然如此请连同下面的诊断一起报告。",
      "splashWaiting": "正在等待 tavotto_open_figure 的结果…",
      "splashWaitingLong": "还没有收到 tavotto_open_figure 的结果（已等 {{seconds}} 秒）。工具已经返回成功的话，是宿主没把结果交给画布：重新打开这张图；仍然如此请连同下面的诊断一起报告。",
      "synced": "已同步"
    },
    "nativeRun": {
      "approve": "运行并连接",
    ...
    "perfProbe": {
      ...
      "saveFailed": "报告没能写进数据目录，这份报告还没有保存。检查磁盘空间或权限后再试一次。",
    ...
    "profiles": {
      ...
      "unset": "未设置",
      "useForCanvas": "用于当前画布",
      "useForCanvasSaveFirst": "先保存这份样式的改动，再用于画布",
  ...
  "errors": {
    "backend": {
      ...
      "bad_inspection": "不认识的导出检查设置：{{value}}。",
      ...
      "dependency_preparation_required": "脚本开跑就需要的包渲染环境里没有，要先准备依赖",
      ...
      "explicit_python_unusable": "你指定的渲染环境用不了；Tavotto 不会自动换成别的环境。请在设置里重新指定，或清除它回到自动选择。",
      ...
      "layout_outside_project": "排版文件只能保存在当前项目的 tavottofile/ 文件夹里；那里指向了项目外（符号链接），这次没有写入。",
      "layout_read_only": "项目文件夹是只读的，排版没有存进项目。改动仍在本机，可以另存到别处或检查文件夹权限。",
      ...
      "script_exited": "脚本调用 sys.exit() 提前结束了（{{error}}）。Tavotto 要的是脚本跑完后留在内存里的图：去掉那句 exit，或只在出错时 exit。",
      ...
      "tiff_bit_depth": "{{file}} 的位深暂不支持。只支持 8 位或 16 位（1 位仅限黑白）。",
      "tiff_color_space": "{{file}} 的颜色模式（如 CMYK、Lab）暂不支持。请转成 RGB 或灰度后再用。",
      "tiff_compression": "{{file}} 的压缩方式暂不支持。请另存为无压缩、LZW 或 Deflate 压缩的 TIFF。",
      "tiff_planar": "{{file}} 的各颜色通道分开存放，暂不支持。请另存为普通（交错存放）的 TIFF。",
      "tiff_sample_format": "{{file}} 的像素是浮点数或有符号整数，暂不支持。请导出为 8 位或 16 位整数的 TIFF。",
      "tiff_unreadable": "{{file}} 读不出来：不是完整的 TIFF 文件。",
    ...
    "engine": {
      ...
      "dependencyPrepareBody": "{{script}} 一开始就要用到的包，渲染环境里没有。Tavotto 可以按项目声明一次装全，再继续打开。",
      ...
      "dependencyPreparePrivatePython": "这台电脑没有可用的 Python。将先下载 Tavotto 自己的 Python {{version}}（约 {{mb}} MB）到 Tavotto 的数据目录，不改动系统与 PATH。",
      "dependencyPreparePrivatePythonCached": "这台电脑没有可用的 Python。将使用已下载并校验过的 Tavotto 自己的 Python {{version}}，不联网。",
      "dependencyPrepareRetry": "重试",
      "dependencyPrepareRun": "准备并继续",
      "dependencyPrepareSkip": "不准备，直接运行",
      "dependencyPrepareState_creating_env": "正在创建 Python 环境…",
      "dependencyPrepareState_downloading_python": "正在下载 Python…",
      "dependencyPrepareState_installing": "正在安装…",
      "dependencyPrepareState_preparing": "正在准备…",
      "dependencyPrepareState_verifying": "正在验证环境…",
      "dependencyPrepareTargetLegend": "安装到哪里",
      "dependencyPrepareTitle": "要先准备 {{count}} 个依赖",
      "dependencyPrepareUnknown": "认不出对应安装包的导入：{{modules}}。这些不会安装；打开后如果仍缺，可手动指定包名。",
      "dependencyTargetHint_project_venv": "安装到 {{venv}}。会修改这个环境，且没有完整回退。",
      "dependencyTargetHint_tavotto_managed": "Tavotto 自己的 Python 环境：新建一代、验证通过后才切换；旧的一代保留到没有会话在用。不改动你的源码和现有环境。",
      ...
      "repairError": {
        ...
        "dependency_consistency_failed": "安装后 pip check 报依赖不一致，环境没有切换。",
        "dependency_hash_mismatch": "下载的包与声明里的哈希不符，环境没有切换。核对锁文件后重试。",
        ...
        "dependency_interpreter_pinned": "渲染解释器已被固定；这条固定还在时，为项目安装的环境不会被使用。",
        ...
        "private_python_cancelled": "已取消准备 Python。",
        "private_python_disk_low": "磁盘剩余空间不足以准备 Python。清理后再试。",
        "private_python_hash_mismatch": "下载的 Python 校验不符，已拒绝使用。请重试。",
        "private_python_invalid_archive": "下载的 Python 归档不合法，已拒绝使用。",
      ...
      "repairPinnedBody": "渲染解释器已固定为 {{python}}（{{source}}）。这条固定还在时，为这个项目安装或选择的环境都不会被使用。",
      "repairPinnedClear": "恢复自动检测",
      "repairPinnedEnvHint": "清除环境变量 {{variable}} 并重新启动 Tavotto 后再试。",
      "repairPinnedEnvSource": "环境变量 {{variable}}",
      ...
      "userEnvLegend": "用哪个 Python",
      "userEnvMissing": "还缺 {{packages}}",
      "userEnvNone": "这台电脑上没有装齐这些包的 Python 环境。",
      "userEnvPartial": "其他找到的环境（还缺包）",
      "userEnvRevert": "改回",
      "userEnvReverted": "已改回原来的渲染环境",
      "userEnvSource_conda": "Conda 环境 {{label}}",
      "userEnvSource_environment_yml": "Conda 环境 {{label}}（项目的 environment.yml）",
      "userEnvSource_login_shell": "你终端里的 Python",
      "userEnvSource_pyenv": "pyenv {{label}}",
      "userEnvSource_python_version_file": "pyenv {{label}}（项目的 .python-version）",
      "userEnvSource_shebang": "脚本第一行指定的 Python",
      "userEnvSource_system": "系统 Python",
      "userEnvSource_vscode": "VS Code 为这个项目指定的 Python",
      "userEnvUse": "改用这个环境",
      "userEnvVersion": "Python {{version}}",
      "workdirChooseAmbiguous": "{{script}} 读的同名数据在脚本目录和项目根目录各有一份，内容不同。选一个运行目录，这个项目只问这一次；Tavotto 不替你猜。",
      "workdirChooseButton": "选择运行目录…",
      ...
      "workdirHintProjectRoot": "脚本在项目根目录里运行：相对路径从项目根找数据；相对路径写出的文件会落进项目目录，并改写同名文件。删除、改名仍会被拦下。",
      "workdirHintSandbox": "开启后在本地环境中运行，兼容性更好。",
      "workdirLabel": "脚本的运行目录",
      "workdirMode_project": "脚本目录",
      "workdirMode_project_root": "项目根目录",
      "workdirMode_sandbox": "沙盒",
      "workdirNowProject": "脚本改为在自己的目录运行，正在重新运行。",
      "workdirNowProjectRoot": "脚本改为在项目根目录运行，正在重新运行。",
      "workdirNowSandbox": "脚本改回在沙盒里运行。",
      "workdirOptionFound": "找得到：{{files}}",
      "workdirOptionFoundNone": "这个目录下找不到脚本读的文件",
      "workdirOptionHint_project": "cwd 是脚本自己的目录：相对路径从脚本旁边找。",
      "workdirOptionHint_project_root": "cwd 是这个项目的根目录：data/x.csv 这样的相对路径从项目根找。",
      "workdirOptionHint_sandbox": "cwd 是 Tavotto 的沙盒：只读的相对路径回退到脚本目录，写出的文件不碰项目。",
      "workdirOption_project": "脚本所在目录",
      "workdirOption_project_root": "项目根目录",
      "workdirOption_sandbox": "继续在沙盒里运行",
      "workdirRecommended": "推荐",
      "workdirRootConfirmBody": "脚本在项目根目录里运行：相对路径读取的数据从项目根找；相对路径写出的文件会像在终端里一样落进项目目录，并改写同名文件。Tavotto 不会替它保存图片，并会拦下删除、改名和移动。只对这个项目生效。",
      "workdirRootConfirmOk": "在项目根目录里运行",
      "workdirRootConfirmTitle": "改为在项目根目录里运行？",
  ...
  "workspace": {
    ...
    "panelBadge": {
      ...
      "nativeInconsistent": "有改动没能还原，重新运行原命令可继续编辑",
    ...
    "save": {
      ...
      "doneLocal": "已保存在本机（没有打开项目）",
      "doneProject": "已保存到项目：{{file}}",
      "failed": "无法保存。改动仍在，请检查磁盘空间后重试。",
      "projectConflict": "项目里的 {{file}} 已在别处改过，这次没有写入。请在弹出的对话框里决定。",
    ...
    "stylePanel": {
      ...
      "boundHint": "上面的改动会改这套样式本身，这张画布上的图跟着变。",
      "building": "正在构建图表…",
      "copiedBuiltin": "内置样式不能改，已复制为「{{name}}」并改用它",
      "copyName": "我的样式",
      "detached": "已脱离「{{name}}」",
      "detachedHint": "这张画布不再跟随样式；重新选中它即恢复。",
      "editCancelled": "样式或画布刚换过，刚才的改动没有生效，请再改一次",
      "emptyAction": "从图层中选择",
      "emptyTitle": "在画布上选中一张图，这里显示它的样式",
      "familyOf": "{{row}}字体",
      "groupLines": "线条",
      "groupText": "文字",
      ...
      "restoreWaiting": "还有图在渲染，渲染完才能恢复",
      "restore_other": "恢复原样 · {{count}} 项",
      "row": {
        "annotation": "画布标注",
        "axisLabel": "轴标题",
        "dataLine": "数据线宽",
    ...
    "topbar": {
      ...
      "projectFileUnsaved": "项目里的 {{file}} 有未保存的改动",
      "readiness": "项目接入状态",
      ...
      "saveError": "保存失败",
      "saveLocal": "已存在本机",
      "saveLocalAt": "已存在本机 {{time}}",
      "saveProjectPending": "未存进项目",
      "saveProjectSaved": "已保存到项目",
      "saveSaving": "正在保存…",
      "saveTitleLocal": "已保存在本机（没有打开项目）。{{mod}}S 保存；⇧{{mod}}S 另存为新排版",
      "saveTitleLocalInProject": "这份排版只存在本机，还没有存进项目。{{mod}}S 存进项目",
      "saveTitleProject": "已保存到项目：{{file}}。{{mod}}S 保存到这个文件；⇧{{mod}}S 另存为新排版",
