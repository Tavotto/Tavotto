...
export default interface Resources {
  ...
  "errors": {
    ...
    "engine": {
      ...
      "workdirConfirmBody": "脚本将在它自己所在的目录里运行：它用相对路径读取的数据文件能直接找到；它用相对路径写出的文件会像在终端里一样落进项目目录。Tavotto 仍然不会替它保存图片，也不会删除或改写项目里已有的文件。只对这个项目生效。",
      "workdirConfirmOk": "在脚本目录里运行",
      "workdirConfirmTitle": "改为在脚本目录里运行？",
      "workdirHintProject": "脚本在自己所在的目录里运行：相对路径读得到数据；它用相对路径写出的文件会落进项目目录。",
