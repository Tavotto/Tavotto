...
export default interface Resources {
  "ai": {
    ...
    "panel": {
      ...
      "emptyHint": "助手会改这张图的脚本；改完可看差异、一键回滚。",
      "generating": "正在生成回答",
  ...
  "dialogs": {
    "closeGuard": {
      "body": "这份文档还有没有写进磁盘的修改。关闭窗口前要先保存吗？",
      "discard": "不保存",
      "save": "保存并关闭",
      "saveFailed": "没能保存成功，窗口先留着。请先处理界面上的保存问题，或选择「不保存」直接关闭。",
      "title": "有未保存的修改"
    },
    "export": {
      "advanced": "高级选项",
      ...
      "locatedHint": "已定位到问题；再点「导出」回到导出面板，填过的设置还在。",
      "mmSize": "{{w}} × {{h}} mm",
    ...
    "layout": {
      "conflict": "磁盘上已经有一份名为「{{name}}」的文档，而它不是本窗口写的（另一个窗口或 Tavotto 之外的工具动过）。",
    ...
    "settings": {
      ...
      "agents": {
        ...
        "codexInstall": {
          "action": "安装 Codex 集成",
          "announce": {
            "failed": "安装没有完成，下面写了卡在哪一步。",
            "ok": "完成。"
          },
          "doctor": "重新诊断",
          "doneNewSession": "装好了。请新开一个 Codex 会话——已经开着的那个会话里验不出新工具。",
          "error": {
            "bad_action": "这个动作不被允许。",
            ...
            "marketplace_state_unknown": "问不到 Codex 的插件市场登记状态（codex plugin marketplace list 没有跑出结论），所以没有动任何东西。这不等于没登记。展开详情看 Codex 报了什么：常见原因是 codex 命令在 Tavotto 这边跑不起来（例如 npm 装的 codex 找不到 node），或 Codex 本身没启动好。",
            ...
            "plugin_state_unknown": "问不到插件的安装状态（codex plugin list 没有跑出结论），所以没有动任何东西。这不等于没装。展开详情看 Codex 报了什么。",
        ...
        "endpoint": {
          ...
          "keyNote": "密钥明文存在本机的配置文件里，只在启动 CLI 时通过环境变量传给它，不会写进命令行或项目文件。",
          "keyNoteDiagnostics": "导出诊断包或复制诊断时，这个字段会被抹成 ***。",
          "keyNoteMore": "存在哪里、能被谁读到",
          "keyNotePerms": "保存后，macOS 与 Linux 上会把配置目录的权限收到仅本人可读；Windows 上不做这一步，请自行确认这台机器只有你在用。",
          "keyNoteWhere": "写在本机配置目录的 config.json 里，明文（与这两家 CLI 自己的做法一致）。",
          "manual": "没有合适的预设，手动填写",
          "modelAdd": "添加",
          "modelDefault": "默认",
      ...
      "diagnostics": {
        ...
        "hidePreview": "收起",
        "nextStep": {
          "engine": "渲染引擎起不来。下面那张渲染环境卡片里有安装与更换 Python 环境的入口。",
          "project_open": "先打开一个项目，与项目有关的那几项才有检查对象。",
          "project_readable": "这个目录现在读不到（可能被移走或删掉了）。重新打开这个项目，或在「全部项目」里换一个。",
          "project_writable": "「写回原始文件」需要这个目录可写。改掉它的权限之后回到这里点「重新获取」。"
        },
        "okDetails": "各项检查结果",
      ...
      "update": {
        ...
        "latestUnknown": "还没有检查过，现在无法判断这是不是最新版本。",
    ...
    "versions": {
      ...
      "restored": "已恢复文档版本「{{name}}」（可撤销；未改动任何源文件）",
      "save": "存版本",
      "saved": "已把当前文档存成新版本",
      "summary": {
  ...
  "errors": {
    "backend": {
      ...
      "no_figures_captured": "脚本跑完了，但没有产生任何图：既没有 savefig，也没有留下 pyplot Figure。展开下面的输出看它自己说了什么——多半是数据文件没找到，或分析在画图之前就结束了。",
      "no_figures_captured_silent": "脚本跑完了，但没有产生任何图，也没有任何输出：既没有 savefig，也没有留下 pyplot Figure。",
      ...
      "unsafe_project_name": "「{{name}}」不是一个合法的目录名：项目名只能是一级目录的名字，不能包含路径分隔符或 ..",
      "unsupported_format": "还不支持导出 {{format}} 格式。",
      "unsupported_script_type": "不是可试运行的 .py 脚本：{{script}}",
      "versions_unreadable": "这份文档的版本历史读不出来，这次的检查点没有写入——磁盘上那份历史一个字节都没动，仍在原处。",
      "workdir_mode_invalid": "不认识的工作目录模式：{{mode}}",
      "worker_build_timeout": "脚本没能跑完，工作进程已重启——请重试。Tavotto 按输出判断卡死：连着 20 分钟没有任何输出，或一直有输出却跑满 4 小时，才会中断。展开下面的输出看它停在哪一步；本来就要静默算很久的，在那一段里打一行进度输出。",
    ...
    "engine": {
      ...
      "projectEnvUsingSystem": "这个项目改用了：{{path}}",
      "projectEnvWhy": "内置环境里没有 {{module}}，已自动改用这个项目自己的环境。",
      "projectEnvWhySystem": "内置环境里没有 {{module}}，你为这个项目选了这台机器上已有的这个环境。",
      "repairBack": "返回",
      ...
      "repairError": {
        ...
        "package_lookup_failed": "没有读懂软件源的回答。可以直接输入包名安装。",
        "package_lookup_not_found": "软件源上没有这个名字的包。查找不做模糊匹配，请核对完整包名。",
        "package_lookup_offline": "连不上软件源。检查网络或你配置的镜像源之后重试。",
        "package_lookup_timeout": "查找超时。稍后重试，或者直接输入包名安装。",
      ...
      "repairSystemRejectedNoMatplotlib": "找到 {{python}} 装了 {{module}}，但它导入不了 matplotlib，没有采用。",
      "repairSystemRejectedUnsupported": "找到 {{python}} 装了 {{module}}，但它的 Python {{version}} 不在 Tavotto 支持的范围内，没有采用。",
      "repairSystemRejectedUnusable": "找到 {{python}} 装了 {{module}}，但它起不来，没有采用。",
      ...
      "workdirConfirmBody": "脚本将在它自己所在的目录里运行：它用相对路径读取的数据文件能直接找到；它用相对路径写出的文件会像在终端里一样落进项目目录，已有的同名文件会被它改写。Tavotto 仍然不会替它保存图片，并且会拦下脚本对项目里文件的删除、改名和移动。只对这个项目生效。",
    ...
    "preflight": {
      ...
      "elementOutsideFigure": "有元素超出图幅 {{mm}} mm，导出时超出的部分会被裁掉",
  ...
  "inspector": {
    ...
    "element": {
      ...
      "colorScaleTip": "色图与色阶上下限是同一份状态：在哪一边改，另一边跟着变。",
      "editElement": "修改图内元素",
      "editProp": "修改{{label}}",
      ...
      "hint": {
        "alpha": "100 % 完全不透明，0 % 看不见；脚本里的 alpha 就是这个 0–1 的值。",
        "aspect": "equal = 一单位数据在 X、Y 方向等长；auto = 填满子图；数字 = Y 比 X 的比例。",
        "bbox": "文字背后的底框：bbox 是它在脚本里的名字。",
        "cbExtend": "色条两端的三角：表示超出范围的值用端点色画。",
        "clim": "色标的取值范围：小于 vmin 的用最低色，大于 vmax 的用最高色。",
        "frameon": "图例背后的那块底框；关掉后底色、边框色、边框不透明度都不再生效。",
        "gridAxis": "X 网格是穿过 X 刻度的竖线，Y 网格是穿过 Y 刻度的横线。",
        "ha": "文字锚点相对文字块的水平位置：改的是「锚在哪」，不是把文字挪开。",
        "invert": "翻转轴的方向，数据不变。",
        "labelpad": "轴标题到刻度文字的距离，单位 pt。",
        "legendAnchor": "bbox_to_anchor：图例框相对子图的锚点坐标，0–1 是子图内，超出即图外。",
        "legendLoc": "loc 是图例在子图内的落位；拖到图外时用锚点定位。",
        "legendSpacingUnit": "单位是字号的倍数，不是 pt：1.0 = 一个字高。",
        "linespacing": "行距是字号的倍数，1.2 = 1.2 倍字高。",
        "majorMode": "主刻度怎么定：自动、按固定步长，或直接给一组值。",
        "markersize": "标记的直径，单位 pt。",
        "minorMode": "次刻度怎么定：自动细分，或按固定步长。",
        "mutationScale": "箭头头部的大小，单位 pt；线宽另算。",
        "ncol": "图例分几列排；只有多于一列时列距才有意义。",
        "scale": "log 只画正数；改缩放会重算刻度，脚本设的刻度值可能不再适用。",
        "scatterSize": "pt² 是面积，不是直径：数值翻一倍，点看上去约宽 1.4 倍。",
        "sizeMm": "整张图的尺寸；改它会让子图按脚本的布局规则重排。",
        "spines": "spine 是子图四条边框线；刻度线是另一样东西，各自可以开关。",
        "stroke": "给文字描一圈边，深色背景上保证可读；用 path effects 实现。",
        "tickDirection": "in 朝图内，out 朝图外，inout 跨在边框上。",
        "tickFormat": "刻度数字的写法，matplotlib 格式串，如 %.1f、{x:.0f}。",
        "tickSides": "刻度线画在哪几条边上；对应的边框线可以独立关掉。",
        "transparent": "导出时图的底色不画；PNG 会是透明背景。",
        "va": "文字锚点相对文字块的垂直位置：baseline 按基线对齐，center 按中线。",
        "zorder": "数字大的画在上面；同一子图里的元素按它决定谁盖住谁。"
      },
      "howAiBody": "直接改写 .py 脚本文件。每次运行前自动留一份快照，任务历史里可以「回滚此次修改」。",
      "howAiTitle": "改图助手的修改",
      "howBothBody": "脚本改动之后，指向已不存在元素的图内修改会被忽略，并在问题面板里提示；其余照常生效。",
      "howBothTitle": "两者相遇时",
      "howItWorksTrigger": "修改保存在哪里？",
      "howOverrideBody": "保存在这份文档里，不改脚本，也不改磁盘上的图。可以撤销；「恢复此元素」「恢复整张图」一次清掉。导出时按全质量重新出图。",
      "howOverrideTitle": "图内修改",
      "howWriteBackBody": "只有这一步会覆盖磁盘上的图。覆盖前自动备份，「历史」可回到任一版本。",
      ...
      "sourceRow": {
        "restoreMenu": "恢复图内修改",
        "restoreMenuTip_other": "恢复图内修改（共 {{count}} 项）",
        "scriptUntouched": "脚本未改动"
      },
      "techDetails": "技术详情",
    ...
    "legend": {
      ...
      "customHintStyled": "示意线用的是你在这里改的样式；恢复链接会撤掉这些修改（一次撤销）。",
      "entriesAria": "图例项列表",
      "entries_other": "图例项（{{count}}）",
      "followHint": "示意线由图中那个对象派生；断开后才能单独改它的颜色、线型和标记。",
      "hide": "隐藏",
      ...
      "spacingUnitNote": "以图例字号为单位：1 em = 一个字号。",
      "stateCustom": "自定义",
      "stateFollow": "跟随图中对象",
      "typography": "文字",
      "unlinkAction": "断开链接",
    ...
    "unsupported": {
      ...
      "legend_anchor_box": "脚本把图例锚在一个矩形范围上（4 个数），这里只改得动锚在一个点上的图例",
      "legend_anchor_transform": "脚本把图例的锚点钉在另一套坐标系上，这里只改得动锚在所属容器上的图例",
    ...
    "writeBack": {
      ...
      "readOnlyTitle": "这个项目关闭了「允许写回原始文件」，可在项目设置里打开",
      "restoreBody": "恢复：面板的「历史」里可回到任一版本；也可直接从备份目录把文件拷回来。生成图的脚本不会被改动。",
      "rewriting": "正在重出…",
      ...
      "topBarReadOnly": "这个项目关闭了「允许写回原始文件」，可在项目设置里打开",
      "topBarShort": "写回",
  ...
  "project": {
    "browser": {
      ...
      "goToPath": "跳转到该路径",
      "nameError": {
        "control_char": "名字里有不可见的特殊字符，请重新输入。",
        "dot_only": "「.」和「..」是目录本身和上一级，不能当项目名。",
        "empty": "请输入项目名。",
        "illegal_char": "项目名是一级目录的名字，不能包含 < > : \" / \\ | ? * 这些字符。",
        "reserved_name": "这是系统保留的名字，换一个。",
