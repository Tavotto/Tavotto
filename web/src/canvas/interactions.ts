 ...
 *
 * 落成哪几条 override 由 `lib/legendScale` 决定（字号与标题字号乘一个倍数；以字号为
 * 单位的间距由引擎随字号等比）；再加一条 `loc_frac` 把不动的那个角钉住——图例的 `loc_frac`
 * 是它左下角的位置，不写的话缩放会绕 loc 预设的那个角进行，而不是用户按住的对角。
 *
 * 预览：与子图缩放不同，这里**做** SVG 预览——字号一变整个图例盒按同一倍数走，matplotlib
