...
/**
 * 色条与它上色的图像 / 集合**共用一份色阶**（审计 T23）：色图、上下限在两边
 * 都能改，改哪边另一边都跟着变（引擎 `ALIAS_GROUPS`，同一份状态的两个 gid）。
 * 这一行把关系说出口，并给一个「选中对方」的入口——两边各显示一次色阶字段
 * 本身没有错，错的是不说它们是同一份。
 *
...
export function colorScalePartner(manifest: Manifest, element: ManifestElement): ManifestElement | null {
...
export function ColorScaleLink({ manifest, element }: { manifest: Manifest; element: ManifestElement }) {
  ...
  return (
    ...
    >
      <Tip label={el('colorScaleTip')}>
        <span className="flex shrink-0 items-center text-ink-3" tabIndex={-1}>
          <Link2 size={12} aria-hidden />
        </span>
      </Tip>
      <span className="min-w-0 flex-1 truncate text-xs text-ink-2">{el('colorScaleLinked', { label })}</span>
      <Button
        size="sm"
      ...
      >
        ...
        {el(partner.role === 'colorbar' ? 'selectColorbar' : 'selectMappable')}
      </Button>
    </div>
  )
}
