  ...
  return (
    <ul className="overflow-hidden rounded-md border border-border bg-surface">
      {agents.map((agent, i) => (
        <li
          key={agent.id}
          className={cn('relative flex min-h-16 items-center gap-3 px-3',
        ...
        >
          ...
          */}
          ...
          <div className="pointer-events-none min-w-0 flex-1 py-2">
            <div className="flex min-w-0 items-center gap-2">
              <span className="truncate text-sm font-medium text-ink">
                {agent.display_name}
              </span>
              <AgentStateBadge state={agent.state} className="shrink-0" />
            </div>
            <p
              className="mt-0.5 truncate font-mono text-xs text-ink-3"
              title={agentSubtitle(agent)}
            >
              {agentSubtitle(agent)}
            </p>
          </div>
