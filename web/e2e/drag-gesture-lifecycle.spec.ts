...
const locate = (page: Page, id: string): Promise<Probe | null> =>
  page.evaluate((gid) => {
...
async function openTitle(page: Page, baseURL: string): Promise<Probe> {
...
test('STATE-04：权威渲染在途时撤销，画布回到原位，拖动那一版晚到也不带回来', async ({ app, page }) => {
  ...
  const afterUp = await locate(page, 'axes_0.title')
  expect(dist(afterUp!, t0), '松手后等权威：预览应当停在拖到的位置').toBeGreaterThan(30)
  hold = false

  await page.keyboard.press('Control+z')
  await expect
    .poll(async () => dist((await locate(page, 'axes_0.title'))!, t0), {
      timeout: 5_000,
...
test('STATE-02：拖动中 Esc 先取消手势，退出图内编辑态后松手不写文档', async ({ app, page }) => {
  ...
  await dragFrom(page, t0, 60, 30)
  await frames(page)
  const during = await locate(page, 'axes_0.title')
  expect(dist(during!, t0), '拖动中标题应当跟着手').toBeGreaterThan(30)

  await page.keyboard.press('Escape')
  const afterEsc = await locate(page, 'axes_0.title')
  expect(afterEsc!.tf, 'Esc 之后预览位移应当还原').toBe(t0.tf)
  expect(dist(afterEsc!, t0)).toBeLessThan(1)
  // 指针还按着：继续移动不复活手势
  await page.mouse.move(t0.x + 80, t0.y + 40)
  await frames(page)
  expect(dist((await locate(page, 'axes_0.title'))!, t0), '取消后继续移动，手势复活了').toBeLessThan(1)

  // 再按：清选中 → 退出图内编辑态
  await page.keyboard.press('Escape')
  await page.keyboard.press('Escape')
  await expect(page.locator('[data-element-svg] svg')).toHaveCount(0)
...
test('STATE-07：拖动中改视图倍率，下一次 move 不跳位、之后跟着指针走', async ({ app, page }) => {
  ...
  await frames(page)
  const later = await locate(page, 'axes_0.title')
