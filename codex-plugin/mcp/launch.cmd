...
rem file LF + ASCII in this batch part, no goto / call :label, probe every candidate by running it.
setlocal EnableExtensions DisableDelayedExpansion
set "TAVOTTO_LAUNCH_PY="
set "TAVOTTO_LAUNCH_PYARG="
if defined TAVOTTO_MCP_PYTHON if exist "%TAVOTTO_MCP_PYTHON%" "%TAVOTTO_MCP_PYTHON%" -c "import sys" >nul 2>nul && set "TAVOTTO_LAUNCH_PY=%TAVOTTO_MCP_PYTHON%"
set "TAVOTTO_LAUNCH_CFG=%TAVOTTO_CONFIG_DIR%"
if not defined TAVOTTO_LAUNCH_CFG set "TAVOTTO_LAUNCH_CFG=%APPDATA%\Tavotto"
if not defined TAVOTTO_LAUNCH_PY if exist "%TAVOTTO_LAUNCH_CFG%\mcp-runtime\venv\Scripts\python.exe" "%TAVOTTO_LAUNCH_CFG%\mcp-runtime\venv\Scripts\python.exe" -c "import sys" >nul 2>nul && set "TAVOTTO_LAUNCH_PY=%TAVOTTO_LAUNCH_CFG%\mcp-runtime\venv\Scripts\python.exe"
if not defined TAVOTTO_LAUNCH_PY py -3 -c "import sys" >nul 2>nul && set "TAVOTTO_LAUNCH_PY=py" && set "TAVOTTO_LAUNCH_PYARG=-3"
for %%N in (python python3) do for /f "delims=" %%P in ('where %%N 2^>nul') do if not defined TAVOTTO_LAUNCH_PY "%%P" -c "import sys" >nul 2>nul && set "TAVOTTO_LAUNCH_PY=%%P"
for /d %%D in ("%LOCALAPPDATA%\Programs\Python\Python3*" "%ProgramFiles%\Python3*") do if not defined TAVOTTO_LAUNCH_PY if exist "%%~D\python.exe" "%%~D\python.exe" -c "import sys" >nul 2>nul && set "TAVOTTO_LAUNCH_PY=%%~D\python.exe"
