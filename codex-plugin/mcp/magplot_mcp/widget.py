...
画布本体是 **Magplot 前端那一份代码**（`web/src/canvas/*` + 现有 stores），
...
* iframe 的 CSP 由 `_meta.ui.csp` 声明，而 Magplot sidecar 的端口是**动态**的
...
RESOURCE_URI = "ui://magplot/canvas/v1.html"
...
def resource_meta() -> dict:
    ...
    return {
        ...
        "openai/widgetDescription":
            "Magplot 交互画布：拖图例与图内文字、改字号线宽刻度、跑出版规范预检、导出矢量 PDF。",
...
def resource_descriptor() -> dict:
    return {
        ...
        "name": "magplot-canvas",
        "title": "Magplot 画布",
        "description": "Codex 内嵌的 Magplot 交互画布（拖拽、字号线宽、预检、导出）。",
