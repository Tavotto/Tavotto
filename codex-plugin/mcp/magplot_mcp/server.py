...
def _tools() -> list[dict]:
    ...
    tools = [
        {
            "name": "magplot_open_figure",
            "title": "打开一张 Magplot 图",
            "description": (
                "打开一个 Magplot 图库里的 matplotlib figure，起一个引擎会话并渲染一次。"
                "返回 session_id、manifest（可编辑元素清单）、预览 SVG、canonical patch "
                "hash 与出版规范。之后用 magplot_apply_overrides 改图、magplot_preflight "
                "体检、magplot_export 出图。"
            ...
            "inputSchema": {
                ...
                "properties": {
                    "project_path": {"type": "string",
                                     "description": "图库目录（含 mm_registry.json）"},
        ...
        },
        {
            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    "out_dir": {"type": "string",
                                "description": "输出目录；缺省 <项目>/magplotfile/export/"},
                    "profile_id": {"type": "string"},
                    "journal": {"type": "object"},
                    "explicit_confirm": {
                        "type": "boolean",
    ...
    if ui:
        ...
        by_name["magplot_open_figure"]["_meta"] = widget.tool_meta(
            invoking="正在打开 Magplot 画布…", invoked="Magplot 画布已就绪")
        by_name["magplot_apply_overrides"]["_meta"] = widget.tool_meta(
...
HANDLERS = {
    "magplot_open_figure": _call_open,
    "magplot_apply_overrides": _call_apply,
    "magplot_preflight": _call_preflight,
    "magplot_export": _call_export,
    "magplot_verify_replay": _call_verify,
    "magplot_close_session": _call_close,
}

#: 结果里挂 UI 的工具（与 `_tools()` 的 `_meta` 一致）
UI_TOOLS = ("magplot_open_figure", "magplot_apply_overrides")
...
def call_tool(name: str, args: dict) -> dict:
    ...
    try:
        ...
        payload = exc.payload()
        # 失败也要机器可读：Codex 得能据此决定下一步（改路径？装 magplot？确认导出？）
        return {"isError": True,
...
class Server:
    ...
    def initialize(self, params: dict) -> dict:
        ...
        return {
            ...
            "capabilities": caps,
            "serverInfo": {"name": SERVER_NAME, "title": "Magplot",
                           "version": _version()},
            "instructions": (
                "Magplot 负责结构化图表编辑：改的是 override（gid + prop + value），"
                "**不会动用户的 Python 源码**。流程：magplot_open_figure 打开 → "
                "magplot_apply_overrides 改（patches 永远发全量列表）→ "
                "magplot_preflight 体检 → magplot_export 出图。"
...
def _self_check() -> int:
    """`python -m magplot_mcp --self-check`：不连 host 也能确认这层是活的。"""
    report = {
        "server": SERVER_NAME,
        "version": _version(),
