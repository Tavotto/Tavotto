...
4. **插件版本 ≠ Magplot 版本。** 当前版本从 `.codex-plugin/plugin.json` 读
   （代码里不写第二份版本号）；`min_magplot_version` 比的是**这次真正为你
   干活的那个 Magplot** 报回来的版本，两者互不代表。

## 缓存

落在 Magplot 的用户配置目录（`handoff.config_dir()`，同一份规则不抄第三遍），
...
* `MAGPLOT_UPDATE_URL` —— 自定义清单地址（自建分发、内网镜像、测试）
...
DEFAULT_URL = ("https://github.com/erwanjun/magplot/releases/latest/download/"
               "codex-plugin.json")
URL_ENV = "MAGPLOT_UPDATE_URL"
DISABLE_ENV = "MAGPLOT_DISABLE_UPDATE_CHECK"
...
def plugin_manifest_path() -> str:
    ...
    scripts/ → magplot-figure/ → skills/ → codex-plugin/ → .codex-plugin/
    """
...
def write_cache(path: str, data: dict) -> None:
    ...
    连 `ValueError` 都要接住：`MAGPLOT_CONFIG_DIR` 是用户给的，里面可能有
...
def check(*, force: bool = False, environ: dict | None = None,
          magplot_version: str | None = None, now: float | None = None,
    ...
    # min_magplot_version 比的是**这次真正为你干活的那个 Magplot**，不是插件自己。
    # 两个版本号各有各的升级节奏，混为一谈会提示用户去升级一个根本没问题的东西。
    required = manifest.get("min_magplot_version")
    if magplot_version and isinstance(required, str):
        too_old = is_newer(required, magplot_version)
        if too_old:
            out["magplot"] = {"status": "too_old", "current_version": magplot_version,
...
def hint(update: dict) -> str | None:
    ...
    if not update or update.get("status") != "available":
        return None
    lines = [
        f"Magplot Codex 插件有新版本：{update.get('latest_version')}",
...
def magplot_hint(update: dict) -> str | None:
    info = (update or {}).get("magplot")
    if not info:
        return None
    return (f"这台机器上的 Magplot 是 {info['current_version']}，"
            f"新版插件要求 {info['required_version']} 或更高："
            "https://github.com/erwanjun/magplot/releases")
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap = argparse.ArgumentParser(
        description="检查 Magplot Codex 插件有没有新版本（只提醒，不下载、不安装）")
    ap.add_argument("--json", action="store_true", help="输出机器可读结果")
    ap.add_argument("--force", action="store_true", help="忽略 24 小时缓存，立刻问一次")
    ap.add_argument("--magplot-version", default=None,
                    help="本机 Magplot 的版本（用于比 min_magplot_version）")
    args = ap.parse_args(argv)

    update = check(force=args.force, magplot_version=args.magplot_version)
    if args.json:
