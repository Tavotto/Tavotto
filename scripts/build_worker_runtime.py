...
def validate_lock(lock: dict) -> None:
    ...
    if not [n for n, t in targets.items() if t.get("shipped")]:
        raise BuildError("锁文件里一个 shipped=true 的目标都没有——"
                         "那样发行链没有任何东西可发")
...
def _validate_target(name: str, target: dict, top_level: list[str]) -> None:
    ...
    if kind not in KIND_SOURCE_PREFIX:
        raise BuildError(f"目标 {name} 的 kind={kind!r} 不认识"
                         f"（只认 {sorted(KIND_SOURCE_PREFIX)}）")
    for key in ("os", "arch"):
    ...
    if not EXACT_VERSION.match(str(py["version"])):
        raise BuildError(f"{name}: python.version 必须是精确补丁版本，"
                         f"拿到 {py['version']}")
    if not str(py["version"]).startswith("3.13."):
        raise BuildError(f"{name}: 内置 runtime 钉死 CPython 3.13.x，"
                         f"拿到 {py['version']}")
    ...
    if not str(py["url"]).startswith(prefix):
        raise BuildError(f"{name}: {kind} 的 CPython 只从 {prefix} 取，"
                         f"不接受其他来源")

    pip = target.get("pip") or {}
    if not isinstance(pip.get("platforms"), list) or not pip["platforms"]:
        raise BuildError(f"目标 {name} 的 pip.platforms 缺失"
                         "——没有它 pip 会按**构建机**的平台装 wheel")
    ...
    for pkg, ver in pkgs.items():
        if not EXACT_VERSION.match(str(ver)):
            raise BuildError(f"{name}: {pkg} 的版本 {ver!r} 不是精确版本"
                             "（不允许范围 / latest / 空）")
...
def get_target(lock: dict, name: str) -> dict:
    try:
        ...
        raise BuildError(
            f"锁文件里没有目标 {name!r}；可用的是 {target_names(lock)}") from None
...
def default_target_name(lock: dict) -> str:
    ...
    for name, target in (lock.get("targets") or {}).items():
        if target.get("os") == want_os and target.get("arch") == arch:
            return name
    raise BuildError(
        f"锁文件里没有 {want_os}/{arch} 的目标（可用：{target_names(lock)}）")
...
ARCH_ALIASES = {"amd64": "x86_64", "x86_64": "x86_64", "x64": "x86_64",
...
def check_runtime_dir(manifest_file: Path, require_smoke: bool,
...
def interpreter(out: Path, target: dict) -> Path:
    ...
    matches = sorted(p for p in bin_dir.glob("python3.*")
                     if not p.name.endswith("-config"))
    ...
    **只删符号链接**：万一上游哪天把 `bin/python3` 换成实体文件，这里必须
    ...
    for name in ALIAS_BINARIES:
        ...
        try:
            ...
            log(f"! 删不掉 {p}: {exc}")
    if removed:
        log(f"✓ 剪掉 {removed} 个解释器别名，打包后省下 "
            f"{freed / 1024 / 1024:.0f} MiB（Tauri 会把符号链接拍平成真副本）")
    return removed, freed


def manifest_dict(lock: dict, name: str, target: dict, build_id: str,
                  built_at: str, lock_sha256: str, smoke: str) -> dict:
...
def extract_windows_embeddable(archive: Path, out: Path, target: dict) -> None:
    ...
    pth.write_text("\n".join(pth_lines(target["python"]["stdlib_zip"])) + "\n",
                   encoding="utf-8")
    log(f"✓ 解压 embeddable 到 {out}，并重写 {pth.name}"
...
def install_packages(out: Path, target: dict, pip_python: str) -> None:
    ...
    for plat in pip["platforms"]:
        cmd += ["--platform", plat]
    cmd += [
        "--python-version", pip["python_version"],
        "--implementation", pip["implementation"],
        "--abi", pip["abi"],
        "--target", str(site),
        "--upgrade",
        *reqs,
...
def can_execute(python: Path) -> bool:
    ...
    **真去跑一次**，而不是拿 `os.name`/`platform.machine()` 去推断：
    ...
    """
    try:
        proc = subprocess.run([str(python), "-c", "print(1)"],
...
def precompile(out: Path, target: dict) -> str:
    ...
    **当场破坏代码签名**，用户下次启动看到的是「应用已损坏」。
    ...
    if not python.is_file() or not can_execute(python):
        log(f"! 跳过预编译（构建机跑不了 {target['os']}/{target['arch']} 的解释器）"
            "——冷启动会慢一些，功能不受影响")
...
def smoke_imports(out: Path, target: dict,
                  top_level: list[str]) -> tuple[str, dict[str, str | None]]:
...
def collect_licenses(out: Path, name: str, target: dict) -> int:
    ...
    if cpython_lic.is_file():
        ...
        index.append(f"- **CPython {target['python']['version']}** — PSF License "
    ...
    if target["kind"] == KIND_MACOS:
        ...
        (lic_root / "cpython" / "UPSTREAM-BUILD.md").write_text(
            ...
            encoding="utf-8")
        index.append("- **CPython 静态依赖**（OpenSSL / SQLite / libffi / libedit / "
...
def _resolve_windows_python(target: dict, python_version: str) -> None:
    ...
    target["python"].update(version=python_version, url=url, sha256=digest,
                            size=size, stdlib_zip=f"{tag}.zip", pth=f"{tag}._pth")
    log(f"✓ CPython {python_version} sha256={digest}")


def _resolve_macos_python(target: dict, release: str,
    ...
    target["python"].update(version=version, release=release, url=url,
                            sha256=digest, flavor=flavor)
...
def resolve_lock(lock: dict, name: str, pip_python: str,
                 python_version: str | None, pbs_release: str | None) -> dict:
    ...
    if target["kind"] == KIND_WINDOWS:
        ...
        if pbs_release or python_version:
            _resolve_macos_python(target, pbs_release
                                  or target["python"].get("release", ""),
                                  python_version)

    pip = target.get("pip") or {}
    with tempfile.TemporaryDirectory() as td:
        report = Path(td) / "report.json"
        cmd = [pip_python, "-m", "pip", "install", "--dry-run", "--quiet",
               "--report", str(report), "--disable-pip-version-check",
               "--no-input", "--only-binary=:all:"]
        for plat in pip.get("platforms") or []:
            cmd += ["--platform", plat]
        cmd += ["--python-version", pip.get("python_version", "3.13"),
                "--implementation", pip.get("implementation", "cp"),
                "--abi", pip.get("abi", "cp313"),
                "--target", str(Path(td) / "unused"),
                *[f"{n}=={target['packages'][n]}" if n in target.get("packages", {})
...
# main
# ---------------------------------------------------------------------------
def build(lock: dict, name: str, out: Path, cache: Path, lock_sha: str,
          pip_python: str, allow_skip_smoke: bool, clean: bool,
          do_prune: bool = True) -> dict:
    target = get_target(lock, name)
    log(f"* 目标 {name}：{target['os']}/{target['arch']}（{target['kind']}）"
    ...
    archive = download(target["python"]["url"], cache / archive_name,
                       target["python"]["sha256"])
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--target", default=None,
                    help="构建哪个目标（默认按本机平台/架构挑）")
    ap.add_argument("--list-targets", action="store_true")
    ap.add_argument("--cache", default=str(DEFAULT_CACHE),
                    help="下载缓存目录（CI 可缓存以省下每次几十 MiB）")
    ...
    ap.add_argument("--python-version", default=None,
                    help="配合 --resolve：换到指定 CPython 补丁版本")
    ap.add_argument("--pbs-release", default=None,
    ...
    try:
        ...
        if args.resolve:
            ...
            lock = resolve_lock(lock, name, args.pip_python,
                                args.python_version, args.pbs_release)
        ...
        build(lock, name, Path(args.out), Path(args.cache), lock_sha,
