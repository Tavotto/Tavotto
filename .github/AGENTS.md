...
| `backend-fast` / `backend-platforms` 的 `--shard`、`tests/support/shard.py`、`shard_weights.json` | pytest 分片（CI03a）→ `pytest-shards.md` | 选项必须 `=` 形式；进程内自验并集 == 全集；不带 `--shard` 是 no-op；权重表只影响平衡不影响覆盖；不加 `-n auto` | `tests/test_merge_queue_workflows.py::TestGates`、`tests/test_pytest_shard.py` |
