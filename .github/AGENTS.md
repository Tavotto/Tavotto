...
| --- | --- | --- | --- |
| `.github/workflows/ci.yml` 的事件 / `needs` / Gate 闭集、`scripts/ci/aggregate_gate.py` | CI 分层 → `ci-lanes.md` | 时机分工与覆盖面不变；三个稳定 Gate；`cancel-in-progress` 只对 PR 开；顶层 `TAVOTTO_NO_TELEMETRY=1`；监听 `pull_request` 的 workflow 都带 `branches: [main]`（新加一个要到 `TestPullRequestBaseFilter` 登记） | `tests/test_merge_queue_workflows.py`、`tests/test_aggregate_gate.py` |
