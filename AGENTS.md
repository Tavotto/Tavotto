...
- `POST /api/ai/run` → spawn `codex exec`（默认）或 `Codex -p`，cwd=figures 目录；
...
- `GET /api/ai/capabilities` 实测本机 CLI（安装/版本/模型/推理强度，两家不同构：
  Codex 无强度选项就不给）；CLI 路径可经 `PATCH /api/ai/settings` 指定，
