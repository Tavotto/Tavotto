...
- **反空门禁纪律**：新增的核心不变式测试，提交前必须手工反证一次——把修复
  ...
  **反证本身也会假绿**——变异没生效、或打在了同名的另一处，看上去就是
  「拿掉却没红」。做法与更多同形状的坑见下面「判据的主语」一节。
...
## 判据的主语（2026-08-22，v0.9 发版的教训）

写门禁/看护时最常犯的错**不是实现写错，是判据问错了主语**——问的范围比该问的
窄，于是它一直是绿的。这比判据写错更难发现。v0.9 发版当天，同一形状出现了十
几次，其中七次是 codex review 替我逮到的：

| 判据当时问的 | 该问的 |
|---|---|
| `ulimit -n` 有多大 | **runner 服务**的限制有多大（它俩方向任意） |
| PATH 上有没有叫 `bash` 的东西 | 有没有**能跑 POSIX 脚本**的 bash（Windows 上那是 WSL 桩） |
| 登录 shell 的 PATH | **服务**的 PATH（三层：systemd → `.path` → `.env`，后面的赢） |
| `/api/version` 通不通 | **带认证的**端点通不通（前者是公共端点，就绪永远成立） |
| 这个文件里有没有裸 Request | **所有**起实例的脚本里有没有 |
| 源码里有没有这个名字 | 有没有真实**调用**（定义与注释都能满足子串） |
| 语法上有没有这个调用 | 这条路**走得到**吗（`if False:`、没人调的 helper） |
| dict 里有没有这个字符串键 | 环境变量**真的被设了**吗（`dict(os.environ, K=v)` 也算） |
| URL 文本里有没有域名 | 目标表达式**引用了谁** |
| 父进程解码对不对 | 子进程**编码**对不对（管道有两端） |
| `stdout` 是不是 PIPE | `stdout` **和** `stderr`（子进程有两条出路） |
| 有没有写 `text=True` | 语言的语义真正是不是文本模式（给了 `encoding` 或 `errors` 就是；裸 `check_output` 回的是 bytes） |
| 有没有未解决的 review thread | 有没有**等着我回**的 thread |

**写完一条判据，先问三句**：这条链上还有哪一端我没管？同类的消费点我扫全了吗？
我判的是**语言/系统真正的语义**，还是「代码里有没有写某个词」？
（后者已有专门记录：修共享判据必须扫全部消费方，`fix-the-predicate-sweep-the-consumers`。）

几条从代价里换来的具体做法：

- **判据尽量写成正面形式。** 「不许出现 `2>/dev/null`」这种否定断言会被自己的
  解释咬到——讲清楚为什么不好，那句注释里就有这个字面量。改成「必须出现
  `> x.log 2>&1`」就免疫了。同理，扫源码前先**剥掉注释与 echo 文案**。
  「判据匹配到散文」这一形状在 v0.9 那天出现了**十次**，四种不同的入口，
  值得逐个认一遍：① 判「这一支里有没有 `daemon-reload` 这几个字」——那一支的
  `printf` 正是在**告诉用户去跑它**；② 判「helper 里有没有 `os.access`」——
  它的 docstring 正在解释**为什么不能用它**；③ 配对判据（有 `chmod` 就得有
  helper）——同一个用例里别处已调过 helper，再多一处裸 `chmod` 照样绿；
  ④ 收紧成「一处都不许有」之后**扫到了自己**：先是自己的注释含那个字面量，
  去掉注释又轮到自己的字符串常量。稳定的写法是三条一起上——**AST 摘
  docstring + 剥注释行 + 判「可执行行」而不是「出现过这几个字」**，
  针本身必要时拼出来避免自指。
- **判源码结构就用 AST，别用子串。** 子串判据在本轮被连破三次（注释满足它、
  函数**定义**满足它、目标写成变量就匹配不到），每补一个洞露出下一个。
  `ast` 找真实 Call 节点、要求一层可达性、剪掉 `if False:` 这种死分支。
- **判不出就别判，并把盲点写在明处。** `**cfg` 展开静态看不出里面有什么，
  那就跳过并在 docstring 里说清楚——假装覆盖到了比不覆盖更坏。
- **精确表达不了判据时，收窄被判对象。** 「排空是否与子进程并发」静态证不了，
  于是改成「启动器一律不许开这个管道」，逃生口整个去掉。
- **静态判据的边界要写出来**：查意外，不是防蓄意（与 playground 完整性校验
  同一裁决）。行为上的真保证在门禁本身，静态那条只是更早更便宜地发现。

**反证也要验落点。** 「反空门禁纪律」要求拿掉修复确认它真的红——本轮两次
「拿掉却没红」其实是**变异脚本自己没生效**（`str.replace` 没匹配上、或打在了
文件里更早的同名串上），我却当成了「看护没用」。所以变异必须先断言目标串存在、
再断言变异落在预期位置，最后才看用例红不红。**每写一条用例立刻变异一次，别攒
到最后**——攒着跑一次冒出四条空门禁，逐条追因比当场发现贵得多。

**从没执行过的门禁不会保持正确，只会安静地烂掉。** v0.9 发版时
`lab_release_gate` 第一次真正跑起来，一口气翻出六层腐坏（runner 标签从没对过、
bootstrap 在 Ubuntu 上从没跑完、dependabot 升 pytest 9 改了 `--collect-only`
的输出格式、三个脚本没跟上会话认证、恢复工具裸调受保护端点、五处开了不读的
管道）。**六条里五条的症状与真实原因毫不相干**——「Lab 在排队」「准备完成」
「标记被删」「起来了又全挂」「隔离实例没起来」「引擎死锁」，每一条单独看都会
被归因到别处。所以：`cancelled` / `queued` **不是结论**，判据应当是「距上一次
**真结论**多久了」。

**诊断输出指错文件，比不输出更坏。** 本轮 bootstrap 的覆盖清单用 `/^# /`
认文件头，把 drop-in **内部**的中文注释也当成了文件名，于是打出「表现是随机的:
LimitNOFILE=…」，而真正该点名的那个文件反倒没出现——读的人会照着去查一个不
存在的东西。诊断是给人在最慌的时候看的，它错一次的代价比它缺席大。同理，
**一个语义错的精确值比一个诚实的粗略值坏得多**：`/proc/<pid>` 的目录 mtime
看着有纳秒，但它未必是进程启动时刻；拿不到亚秒精度就把判据做保守（宁可假红）
并写明为什么，别用一个语义不对的量装严密。

**内核层的 wchan 只告诉你在等什么原语，不告诉你在哪一行等。** 本轮据此把
harness 的管道死锁误判成产品死锁（8 线程 futex_wait + 1 个 pipe_write，
看起来就是引擎问题），是 py-spy 的 Python 栈把它钉到 `logging.emit` 上的。
拿不到语言层的栈就别下结论。
